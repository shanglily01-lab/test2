...
- 权威：`docs/REQUIREMENTS_LOGIC_ZH.md` §7.3（v4.5.6）
...
- **退出（v4.5.7）**：按币评估 SL/TP/hold（地板 SL≥2.5%、**上限 4.5%**/TP≥3%；失败 fallback 4.5/8/6）+ trail 激活 **≤1.0%** + **soft 无跟进**（45min / 峰≤0.5% / 浮亏≤-1.2%）；peak 落库/恢复；关战略平仓与持仓顾问；**不做**旧 ai-trail
