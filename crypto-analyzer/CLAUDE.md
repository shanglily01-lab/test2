...
- **入场（v4.5.22 / v4.5.35 / v4.5.47 / v4.5.48 / v4.5.49 / v4.5.50 / v4.5.51 / v4.5.52 / v4.5.54 / v4.5.55 / v4.5.56）**：A1 只开 **趋势新高回踩**（`15m_trend_high_pullback`，离高 0.80%–3.50%）；回踩后从该低点抬起仍是 A1；无新高的反弹/不过前高不得当 A1（走 B3 打标）；**Big4 LONG 禁 A2/B2/B3**；**C3 须 Big4 LONG**；**C4** 须离高拒绝才空（小仓 ×0.20）；**C1 破位跟风**（×0.35）；A2 弱反抽拒绝后开空；BRAIN 强制限价；**15m RSI 管时机**（多单不追超买，摸顶空等拐头）
...
### 现货镜像（REQ-SPOT）【v4.5.26 · 仅模拟】
- 权威：`docs/REQUIREMENTS_LOGIC_ZH.md` §7.4
- 合约模拟 **FILLED** 后跟开 `spot_positions`（account_id=3）：**BRAIN A1** + **DeepSeek LONG**；**仅 L0**；**500U/单**；最多 5 仓
- **不跟** 空单 / C3 突破 / 破位 / 旧 DCA；`spot_trading_enabled` 缺省关
- **现货实盘** `spot_live_enabled`：成交瞬间市价买/卖；打开不回填；不进合约 `LIVE_SYNC_SOURCES`
- 持仓监控在 `crypto-app-main` 每 1min；回归：`validate_spot_paper.py`
...
- 限价：做多 A1 挂 **15m 回调区**；**C3/C1 破位市价跟风**（**C3 须 Big4 LONG**；禁 RSI 极端+7日高追；止盈后 4h 冷却）；B2 须反抽失败后再市价跟；**B3 须 Big4 非 LONG** 且顶部第一回调后挂空；A2 须离低点并拒绝后挂反抽区；**15m RSI 管时机**（不定方向）；**持仓不限时**（低点买高点卖）；SL **C1/B2 2.0% / C3 2.5% / A1 3.0%**；−120U；5x / 1000U
...
| `validate_spot_paper.py` | **REQ-SPOT** 镜像资格 / 非实盘 / DCA 已停 |
...
- **合约自选**（§7.5）：`/watchlist`；价格优先浏览器直连币安合约 WS，3s 无 tick 则 `/api/watchlist/prices` 1s 补价；下单/触价仍 ticker；限价可撤；实盘随 `live_trading_enabled`，仅 L0，成交瞬间同步，打开不回填
