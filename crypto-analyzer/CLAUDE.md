...
- **入场（v4.5.22 / v4.5.35 / v4.5.47 / v4.5.48 / v4.5.49 / v4.5.50 / v4.5.51 / v4.5.52 / v4.5.54）**：A1 只开 **趋势新高回踩**（`15m_trend_high_pullback`，离高 0.80%–3.50%）；回踩后从该低点抬起仍是 A1；无新高的反弹/不过前高不得当 A1（走 B3 打标）；**Big4 LONG 禁 A2/B2/B3**；**C4** 须离高拒绝才空（小仓 ×0.20）；**C1 破位跟风**（×0.35）；A2 弱反抽拒绝后开空；BRAIN 强制限价
...
- 限价：做多 A1 挂 **15m 回调区**；**C3/C1 破位市价跟风**（C3 禁 RSI 极端+7日高追；止盈后 4h 冷却）；B2 须反抽失败后再市价跟；**B3 须 Big4 非 LONG** 且顶部第一回调后挂空；A2 须离低点并拒绝后挂反抽区；C3/C1/B2/B3/C4 持仓 **4h** / A1/A2 **6h**；SL **6%** / TP **3%** / 5x / 500U
