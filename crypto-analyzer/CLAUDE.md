...
- **入场（v4.5.22 / v4.5.35 / v4.5.47）**：LONG 等 15m **从高点回踩**（须 `15m_pullback_from_high`；离高 <**0.80%** 不挂；低点反弹不得当 A1）；**Big4 LONG 禁 A2/B2**；**B3/C4 须离高拒绝确认后才空**（小仓 ×0.20）；**C1 破位跟风**（×0.35）；A2 弱反抽拒绝后开空
