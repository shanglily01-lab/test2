...
2. **🔥 同向多单**: 同一交易对同一方向**允许开2个单**（V2一个，V3一个）
   - 例如: BTC/USDT LONG 可以同时有 v2持仓 + v3持仓
   - 系统会检查 `signal_version` 字段避免重复开仓
3. **信号冲突**: 同一币种可能同时产生V2和V3信号，会分别开仓
4. **数据收集**: 至少运行3天再做对比分析，1天数据样本太少
5. **参数调整**: 测试期间不要频繁调整参数，保持稳定观察
