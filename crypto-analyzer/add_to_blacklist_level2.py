...
if blacklist:
    for row in blacklist:
        ...
        print(f'{symbol:15} Level {level} | {margin}x倍率 | {trades}笔 | 胜率{float(wr):.1%} | 盈亏${pnl:.2f}')
        if reason:
            print(f'  原因: {reason}')
else:
    print('当前无Level 2+黑名单交易对')

print('\n=== 准备添加的交易对 ===')
# 要添加的垃圾交易对 (rating_level=2, margin_multiplier=0.125)
blacklist_level2 = [
    ('1000PEPE/USDT', 4, 1, -26.57, 25.0, '4笔订单，1赢3输，胜率25%，亏损$26.57'),
    ('TRB/USDT', 4, 1, -29.66, 25.0, '4笔订单，1赢3输，胜率25%，亏损$29.66'),
    ('FOGO/USDT', 5, 2, -59.78, 40.0, '5笔订单，2赢3输，胜率40%，亏损$59.78'),
...
for symbol, total_orders, win_orders, total_pnl, win_rate, reason in blacklist_level2:
    ...
    if existing:
        ...
        if existing_level < 2:
            ...
            cursor.execute('''
                ...
                SET rating_level = 2,
                    ...
                    level_change_reason = '表现极差，强制升级到黑名单Level 2',
                    stats_start_date = %s,
                    stats_end_date = %s,
                    updated_at = NOW()
                WHERE symbol = %s
            ...
            print(f'✅ 升级: {symbol} (Level {existing_level} -> Level 2)')
            updated_count += 1
        else:
            print(f'⏭️  跳过: {symbol} (已在Level {existing_level})')
    else:
        # 新增
        cursor.execute('''
        ...
        print(f'✅ 新增: {symbol} (Level 2)')
        added_count += 1

conn.commit()

print(f'\n=== 执行结果 ===')
print(f'新增: {added_count}个交易对')
print(f'升级: {updated_count}个交易对')
print(f'总计: {added_count + updated_count}个交易对已加入黑名单Level 2')
...
''')
level2_list = cursor.fetchall()
for row in level2_list:
...
print('\n✅ 完成！这些垃圾交易对已被关入黑名单Level 2，保证金倍数降至0.125x')
