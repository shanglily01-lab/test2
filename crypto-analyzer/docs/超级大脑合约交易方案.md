...
完成了K线强度交易优化系统的4个核心模块：

#### 模块1: K线强度评分器 (KlineStrengthScorer)
**文件**: [app/analyzers/kline_strength_scorer.py](../app/analyzers/kline_strength_scorer.py)
...
**关键方法**:
- `calculate_strength_score()`: 计算综合评分
- `get_entry_strategy()`: 获取入场策略参数
- `format_score_report()`: 生成可读报告

#### 模块2: 信号决策大脑 (SignalDecisionBrain)
**文件**: [app/analyzers/signal_decision_brain.py](../app/analyzers/signal_decision_brain.py)
...
#### 模块3: 智能入场执行器 (SmartEntryExecutor)
**文件**: [app/services/smart_entry_executor.py](../app/services/smart_entry_executor.py)
...
#### 模块4: 智能平仓优化器 K线监控方法
**文件**: [app/services/smart_exit_optimizer_kline_methods.py](../app/services/smart_exit_optimizer_kline_methods.py)
...
**触发条件**:
1. **1H K线反转**: 净能量反向>=3 → 平仓50-70%
2. **15M强力反转**: 连续强反向信号 → 全部平仓
3. **持仓到期+强度衰减**: 评分<15 → 平仓50-100%
4. **盈利+强度减弱**: 盈利≥2% & 评分<20 → 平仓70-100%
5. **亏损+方向反转**: 亏损>1% & 方向反转 → 止损100%
...
#### K线数据获取脚本
**文件**: [fetch_kline_data_5days.py](../fetch_kline_data_5days.py)
...
**数据存储**:
- 表名: `kline_data`
- 字段: symbol, exchange, timeframe, open_time, timestamp, open_price, high_price, low_price, close_price, volume
- 去重: UNIQUE KEY (symbol, exchange, timeframe, timestamp)

**执行统计**:
```
总任务数: 64 (16交易对 × 4周期)
成功获取: ~10,000+ K线数据
数据更新: 支持 INSERT ... ON DUPLICATE KEY UPDATE
```

### 1.4 测试验证

#### 测试脚本
**文件**: [test_kline_strength_scorer.py](../test_kline_strength_scorer.py)
...
### 2.1 数据库优化
- 修复了K线数据插入时缺失 `open_time` 字段的问题
- 优化了时间戳转换逻辑 (毫秒 → DATETIME)
- 添加了批量数据更新支持
...
新流程:
信号检测 → K线强度评分 → 智能决策大脑 → 动态分批建仓 → K线强度监控 → 智能分批平仓
```
...
**入场决策**:
```
1. SignalAnalysisService.analyze_kline_strength()
   ↓ 获取三周期K线强度
2. KlineStrengthScorer.calculate_strength_score()
   ↓ 计算综合评分
3. SignalDecisionBrain.should_open_position()
   ↓ 决策是否开仓
4. SmartEntryExecutor.execute_entry()
   ↓ 动态分批建仓
```

**平仓决策**:
```
1. SmartExitOptimizer._check_kline_strength_decay()
   ↓ 检测K线强度变化 (每15分钟)
2. 评估5种平仓触发条件
   ↓
3. SmartExitOptimizer._execute_partial_close()
   ↓ 执行部分/全部平仓
```
...
### 5.1 待集成功能
- [ ] 将K线强度监控集成到SmartExitOptimizer主类
- [ ] 在实盘引擎中启用K线强度决策
- [ ] 添加K线强度历史回测
...
### 新增文件
- `app/analyzers/kline_strength_scorer.py` - K线强度评分器
- `app/analyzers/signal_decision_brain.py` - 信号决策大脑
- `app/services/smart_entry_executor.py` - 智能入场执行器
- `app/services/smart_exit_optimizer_kline_methods.py` - K线监控方法
- `fetch_kline_data_5days.py` - K线数据获取脚本
- `test_kline_strength_scorer.py` - 评分器测试脚本
- `KLINE_STRENGTH_TRADING_OPTIMIZATION.md` - 完整实施文档

### 修改文件
- `config.yaml` - 新增16个测试交易对
- `docs/超级大脑合约交易方案.md` - 本文档
...
```bash
# 提交1: K线强度系统完整实现
git commit -m "feat: 实现K线强度评分交易优化系统
...
# 提交2: 修复K线数据获取脚本
git commit -m "fix: 修复K线数据插入时open_time字段缺失问题

- 修正save_klines()方法，添加open_time字段
- 优化时间戳转换逻辑
- 成功获取16个交易对×4周期的K线数据
...
通过K线强度评分系统，实现了从"固定策略机械开平仓"到"动态评估智能决策"的质变，为超级大脑交易系统注入了真正的"智能"。
# 第四部分: K线强度驱动的智能分阶段平仓系统 (2026-01-27)

## 📋 概述

在第三部分实现的分批建仓和基于计划时间的分批平仓基础上,现在引入**基于K线强度衰减的智能分阶段平仓系统**。这是一个更智能、更动态的平仓策略,不再依赖固定的平仓时间,而是根据市场K线强度的实时变化来决定平仓时机和比例。

### 核心理念

**从"计划时间驱动"升级为"市场状态驱动"**

| 对比维度 | 旧版(第三部分) | 新版(第四部分) |
|---------|--------------|--------------|
| **触发机制** | 计划平仓前30分钟 | K线强度实时监控 |
| **平仓依据** | 价格基线分位数 | K线强度评分+衰减检测 |
| **平仓阶段** | 2批固定(50%/50%) | 3阶段动态(50%→70%→100%) |
| **决策智能** | 价格位置 | 市场趋势+持仓状态 |
| **适应性** | 固定时间窗口 | 动态响应市场变化 |
...
```python
net_power = 强多K线数量 - 强空K线数量

强多K线: 涨幅 > 1.5% 且 成交量 > 平均成交量*1.2
强空K线: 跌幅 > 1.5% 且 成交量 > 平均成交量*1.2
```
...
```python
total_score = (
    weight_1h * net_power_1h +
    weight_15m * net_power_15m +
    weight_5m * net_power_5m
)
...
1. **1H K线反转**
   - 做多持仓: strength_1h['net_power'] <= -3
   - 做空持仓: strength_1h['net_power'] >= 3
   - 1小时周期出现明显反向力量
   - 说明趋势可能转向
   - **动作**: 先平50%锁定利润/止损

2. **盈利≥2% + 强度大幅减弱**
   - 条件: profit_pct >= 2.0 AND total_score < 15
   - 已经有不错的盈利
   - 但市场强度明显减弱
   - **动作**: 部分止盈,保留仓位观察
...
持仓已经部分平仓50%,剩余50%继续观察。

**触发条件**:

1. **1H K线反转加剧**
   - 做多: strength_1h['net_power'] <= -5
   - 做空: strength_1h['net_power'] >= 5
   - 反转力量加强到±5
   - 趋势确认转向
   - **动作**: 再平70% (即剩余仓位的70%)

2. **盈利≥4% + 强度减弱**
   - 条件: profit_pct >= 4.0 AND total_score < 20
   - 盈利已经不错
   - 强度开始减弱
   - **动作**: 大部分止盈

3. **持仓4小时 + 强度衰减**
   - 条件: hold_minutes >= 240 AND total_score < 15
   - 持仓较长时间
   - 强度持续低迷
   - **动作**: 基于时间的托底平仓
...
在任何阶段,以下条件会立即触发**全部平仓**,无视当前stage:

### 1. 15M连续强力反转
- 做多持仓: strength_15m['net_power'] <= -5 AND strength_5m['net_power'] <= -5
- 做空持仓: strength_15m['net_power'] >= 5 AND strength_5m['net_power'] >= 5
- **说明**: 15分钟和5分钟都出现强力反向K线,市场快速反转

### 2. 亏损>1% + 方向反转
- 条件: profit_pct < -1.0 AND current_direction != position_direction
- **说明**: 已经亏损且K线方向完全反转,立即止损

### 3. 固定止损/止盈
- 止损 (持仓≥1小时): 做多: current_price <= stop_loss_price / 做空: current_price >= stop_loss_price
- 止盈 (任何时候): 做多: current_price >= take_profit_price / 做空: current_price <= take_profit_price

### 4. 顶底识别 (持仓≥2小时)
- 识别到顶部(做多) 或 底部(做空) → 立即全部平仓

### 5. 6小时绝对托底
- hold_minutes >= 360 → 无论盈亏,全部平仓
...
```
T+0H: $86,000 → 开仓
T+1H: $87,200 (+1.4%) → K线强劲,继续持有
T+2H: $88,500 (+2.9%) → 达到+2.9%盈利
...
✅ 触发: "1H K线反转加剧"
📤 平仓70%剩余 @ $87,800
💰 锁定: ($87,800-$86,000)×0.007 = +$12.60

[继续下跌]
T+5.5H: $87,200 (+1.4%)
✅ 触发: "持仓5小时托底"
📤 平仓剩余15% @ $87,200
💰 锁定: ($87,200-$86,000)×0.003 = +$3.60

总盈利: $26.50 + $12.60 + $3.60 = $42.70
平均平仓价: $88,295
ROI: +12.4%
```
...
| 策略 | 平仓价 | 盈利 | ROI |
|------|--------|------|-----|
| 固定止盈(+5%) | $90,300 | 未触及,继续持有 | - |
| 回撤止盈(从高点-2%) | $86,726 | $14.52 | +4.2% |
| **新策略(分阶段)** | **$88,295** | **$42.70** | **+12.4%** |
...
1. **从时间驱动到市场驱动**: 不再依赖固定平仓时间,而是实时监控市场K线强度变化

2. **三阶段分批平仓**: 50% → 70% → 100%,逐步减仓而非一次性平仓

3. **多层决策体系**: 7个优先级检查,确保在任何情况下都能合理平仓

4. **数据驱动决策**: 所有平仓决策基于实时K线数据和量化评分
...
完整闭环:
信号检测 → 强度评分 → 智能开仓 → K线监控 → 分阶段平仓
```

通过这套K线强度驱动的智能分阶段平仓系统,真正实现了"让市场告诉我们何时平仓",而不是机械地执行固定策略。

---

## 相关文件

- `app/services/smart_exit_optimizer.py` - 智能平仓优化器主文件 (lines 1040-1350)
- `app/services/smart_exit_optimizer_kline_methods.py` - K线强度检查方法
- `app/analyzers/kline_strength_scorer.py` - K线强度评分器
- `app/analyzers/signal_analysis_service.py` - 信号分析服务
- `smart_trader_service.py` - 交易主服务 (lines 1124-1144: 事件循环修复)

---

**本次更新时间**: 2026-01-27
**实施状态**: ✅ 已上线运行
**Git Commit**: cb9311b (事件循环修复), 3f14763 (黑名单表修复), 6040dff (余额字段修复)
