...
- **`deepseek_explore`**: `_sync_to_live()` when `check_live_open_allowed()` passes (see §11).
- **Gemini / GPT explore**: paper only.
...
**Source allowlist (open only):** `deepseek_explore`, `deepseek_predict`. Gemini/GPT and all other strategies stay paper-only for live opens even if `live_trading_enabled=1`.

**Beijing-time open windows:** opens are allowed only during **10:00-16:00** and **22:00-next day 04:00** Beijing time; on the UTC server these are **02:00-08:00** and **14:00-20:00**. Both paper and live opens are gated by `trading_gates.get_beijing_open_window_status`.
