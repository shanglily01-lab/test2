...
**中线策略**（`gemini_midline_*` / `deepseek_midline_*`）：量化扫描，**跳过**开仓/持仓顾问；**不参与** `SmartExitOptimizer` 监控（由 `position_sl_tp_monitor` 负责**仅硬 SL/TP**、到期/爆仓；**不含 ai-trail-tp**）；模拟保证金 500U；**限价偏移 做多−3% / 做空+3%**、**6 小时未成交过期**；**限价成交后**经 PaperLimitSync 同步实盘（须 `live_trading_enabled=1` + L0 白名单）；实盘保证金同 API `max_position_value` × 评级比例；杠杆/SL/TP 同策略常量；模拟平仓时须 `live_close_enabled=1` 且 source ∈ LIVE_SYNC_SOURCES 才同步平交易所。
...
| 预测 | every(2~8).hours 轮询 + every(5).min | 固定槽位（锚点北京 21:30）+ `*_predict_next_due_utc` 认领；周期=max_hold_hours |

**槽位逾期补跑**：`now < 本周期槽` 时，若上一槽尚无 `status=ok`，须放行逾期补跑（禁止仅因「未到本槽」挡死上一漏槽，例如 3h 周期下 07:30~08:45 漏掉 05:45）。
...
- 主预测（Gemini/DeepSeek）候选池：`candidate_pool_snapshot` 技术面 **TOP50**（与探索 `prepare_universe_for_llm` 一致）；**禁止**对 `price_stats_24h` 全市场（500+）逐 symbol 回退扫 `kline_data`（易 MySQL 2013 超时）  
- DeepSeek 预测：symbol 数据建完后再 `predict_claim_next_slot`，早期失败可由 5min 轮询重试  
...
| Gemini/DeepSeek/GPT 探索 | 固定槽（锚点 21:30 北京）+ 10min 轮询 |
| Gemini/DeepSeek/GPT 预测 | 固定槽 + 5min 轮询 |
| 战术 15 槽位 | 15 min |
| Gemini 情绪 | 8 h |
| MySQL EVENT `update_all_coin_scores` / `calculate_coin_score` | **已下线**（migration 025 DROP EVENT+PROCEDURE） |

探索页首屏：`GET /api/gemini-explore/bootstrap` / `GET /api/deepseek-explore/bootstrap` **单连接**返回 status+runs+open+stats；**独立 API 连接池 20**（`read_timeout=5s`）。池内 idle **>30min** 连接 checkout 时**按时间丢弃、禁止 ping**（隔夜死连接 ping 会占满池导致次日首屏卡死）。首屏成功后 **90s 内暂停轮询**；中线 `/params` 仅切到中线 Tab 时加载。
...
| REQ-MIDLINE | `midline_swing_config.py`, `midline_explore_worker.py`, `midline_swing_scanner.py`, `midline_swing_api.py` |
