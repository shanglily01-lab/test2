...
> **冲高不过前高（v4.5.48）**：近 32–48 根 15m 已有高顶 → 回落 ≥0.40% → 第二次冲高低于前顶 ≥0.20% → `15m_failed_retest`，打 **B3 做空**（Big4 仍 LONG 也允许）。禁止把第二次较低的顶当 A1 回踩。`entry_timing` 记 `failed_retest_short` / `exhaustion_ready`。  
...
| 参数 | 值 |
|------|-----|
| 限价偏移 | **A1/A2 仍限价**：做多回调区 / 做空反抽区；无区时回退 ±1%。**C1/C3/B2/B3/C4 市价跟风**（不挂限价） |
...
- **做多 A1（v4.5.49）**：须同时满足：1h EMA 多头 + HH/HL；15m **对近 32 根趋势新高回踩**（`15m_trend_high_pullback`：回踩的高点 ≥ 前 16 根高点，离高 **0.80%–3.50%**，未破前段低点）；缩量或下影；回到 EMA20 / 38–50% 区才挂限价。未就绪记 `wait_pullback` / `not_trend_high_pullback`。贴高浅跌、破位阴跌、`15m_higher_low` 单独都不够。**从低点抬起** → `bounce_not_pullback`。**第二次冲高不过前高** → `failed_retest_short`（走 B3）。滞涨结构 → `stall_high_not_pullback`。
- **同币止盈冷却（v4.5.25）**：同 symbol、同方向，近 **4h** 已平仓且 `realized_pnl>0` → 破位/BRAIN 均不得再开（`recent_tp_cooldown`）。去重仍拦当前持仓；止盈后再追由本条拦。
- **做空 B3/C4（顶部回调点）**：先贴着近 8 根高点，再等 **第一根离开高点的拒绝/回调**：离高 **≥0.28%** 且收在 K 线下半（上影或阴线）。**已有高顶、第二次冲高不过前高**（`15m_failed_retest`）在失败高点附近即可挂空，不必再等 0.28% 第一回调。仍放量/收在上沿**破前顶** → `wait_stall`；只在高点横盘尚未真正离开且无失败反抽 → `at_high_wait_first_callback`；已离高 ≥1.35% → `missed_high`。中线确认后 **市价空**；BRAIN 仍限价挂在高点上方 0.20–0.55%。**已有真实拒绝或不过前高时允许 4h 仍偏多**；仅 `stall_at_high` / 停新高 / 贴 7 日高 **不得**当衰竭。
...
| ID | 名称 | 方向 | 一句话描述 |
|----|------|------|------------|
| **A1** | 多头趋势回踩 | LONG | **v4.5.49**：1h EMA 多头 + HH/HL + 对趋势**新高**回踩（`15m_trend_high_pullback`，离高 0.80%–3.50%，不破前低，缩量/下影）。**禁止**反弹、不过前高、破位阴跌 |
...
| **B3** | 暴涨滞涨回落 | SHORT | 急涨后涨不动，或**已有高顶后第二次冲高不过前高**（`15m_failed_retest`）→ **在失败高点挂空**（不等回落到 EMA） |
...
**B3 暴涨滞涨做空 — 必选清单（v4.5.34 / v4.5.48）**：

1. 位置：现价距近 8 根 15m 高点 ≤ **0.85%**（冲高区，不是已回落的中段）
2. 「涨不动」证据须含 **拒绝**：长上影、价新高量萎缩、RSI 15m **从超买拐头**、假突破收回；**或** `15m_failed_retest`（先有高顶，回落后再冲但不过前高）
3. 卖点：普通滞涨须已离高 **≥0.28%** 且当根收在下半；**不过前高**在失败高点附近即可挂空。限价挂在现价上方 **0.20–0.55%**；**禁止**等回落到 EMA 再空
4. 等待：最后一根仍收在上沿**创新高（破前顶）** → `wait_stall`；贴高横盘尚未离开且无失败反抽 → `at_high_wait_first_callback`
5. 放弃：已离高点 ≥ **1.35%** → `missed_high`；放量再破前高 → `invalidated`
6. 禁止 B3：4h 仍明确偏多且**没有**真实拒绝、也没有不过前高（趋势未尽，不在高点摸顶）
