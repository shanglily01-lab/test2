...
**其余**（GPT 探索/预测、战术、反转、中线、smart_trader、BTC 动量等）**仅模拟仓**。
...
- L3 模拟：`blacklist_level3_enabled` 固定 enforced  

### 6.5 实盘保证金

- 来源：`user_api_keys.max_position_value` × `get_live_margin_ratio(symbol)`  
...
| 2 | PENDING 限价 → executor 触价或超时转市价 → FILLED |
| 3 | 成交瞬间 §4.2 决定 live_sync_status |
| 4 | PaperLimitSync 在 5 分钟窗内同步实盘（若 NULL） |

**中线策略**（`gemini_midline_*` / `deepseek_midline_*`）：量化扫描，**跳过**开仓顾问；**仅模拟**；限价偏移 3%，**限价单 6 小时未成交过期**（`MIDLINE_LIMIT_TIMEOUT_MINUTES=360`）。
...
**实现**: `open_advisor_strategy_rubrics.py`、`gemini_position_advisor.py`  
**路由**: `open_advisor_routing.py` — 探索/预测等按 source 选 rubric  

| 类型 | 节奏 | 核心判据 |
|------|------|----------|
| 开仓顾问 | 模拟开仓前（部分策略可 skip LLM） | **15m 趋势 + 量价** 与 side 一致 |
| 持仓顾问 | 满 15min 后每 15min | **15m 表** 主审；1h 交叉验证 |
...
| 等级 | 条件概要 | 实盘 |
|------|----------|------|
| L0 白名单 | 盈利+胜率双达标 | 可（仅 L0） |
| L1 | 盈利或胜率单项 | 禁止 |
| L2 | 轻度亏损区间 | 0.125x 保证金（若允许） |
| L3 | 双差 | 模拟也禁止 |
...
| REQ-ADVISOR | `open_advisor_strategy_rubrics.py`, `gemini_position_advisor.py` |
