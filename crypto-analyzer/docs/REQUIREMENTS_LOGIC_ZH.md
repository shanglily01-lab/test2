...
> **BRAIN / 中线卖点（v4.5.16）**：B3/C4 **冲高滞涨**在高点挂空（上影/量价背离/不再创新高 ≥2 条才 `exhaustion_ready`）；仍加速则 `wait_stall`；已离高点则 `missed_high`。C1/A2 仍等回抽拒绝。  
...
- **做空 A2/C1**：仍等 15m 回抽到破位位/EMA 拒绝，禁止杀跌当根追空。
...
扫描 L0+L1（轮询 tick）
  ...
  → **入场点** `entry_timing`：
       ...
       仍加速 → `wait_stall`；已离高点 → `missed_high`；C1/A2 等回抽拒绝
  → create_paper_limit_order → 多单挂回调区；B3/C4 空单挂冲高区（非等 EMA）
  → 触价后重跑成交安全闸门；拒绝即 EXPIRED
  → 30min 未成交取消，禁止转市价
  → OPENED = 已挂 PENDING 限价；成交后才生成持仓
```

**入场（v4.5.16 买点/卖点）**：C3 confirmed 须回踩（缩量 / 15m HL / 下影）。B3/C4 **在高点卖**：`stall_at_high` / 上影 / 量价顶背离 / RSI 拐头 ≥2 条才 `exhaustion_ready`；禁止把仍在放量创新高当成卖点。C1 confirmed 须回抽拒绝。未确认的 C 族仍可识别，但 BRAIN **等到 `entry_timing.ready`** 才下单。A1 本身即回踩买点，仍须不在冲高延伸区。
