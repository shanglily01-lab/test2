...
| 战术 15 槽位 | 15 min |
| Gemini 情绪 | 8 h |
| MySQL EVENT `update_all_coin_scores` | **每 15 min**（migration 024；禁止 5min） |

探索页首屏：`GET /api/gemini-explore/bootstrap` / `GET /api/deepseek-explore/bootstrap` **单连接**返回 status+runs+open+stats；API 连接池 **20**。禁止首屏并行打满连接池导致整站卡死。
