...
> **盈亏比（v4.5.62）**：禁止 0.4–0.8% 碎止盈对 6%/−300U 止损。锁利须先有 ≥1.80% 浮盈；浮盈回吐须峰值 ≥2.20%。破位硬 SL：C1/B2 **2.0%**、C3 **2.5%**、A1 等 **3.0%**；已开 6% 单用 **−120U** 熔断。C1/C3/B2 不因 `missed_low/high` 平。每轮跟风最多 **3** 笔。  
> **结构持仓（v4.5.57 / v4.5.60）**：BRAIN / 破位 **不再 4–6h 到期强平亏损单**。做多：合适低点买、合适高点卖；做空相反。优先 15m 衰竭高 / 拒绝低；**错过结构点仍止盈、浮盈回吐锁利**，禁止把盈利单扛到硬 SL。硬 SL、BRAIN −60U、破位 −120U 仍兜底。硬 TP、5m 早撤仍关。持仓顾问：结构点未到不得因时长建议卖；锁利由程序执行。  
...
> **C3 前提（v4.5.56）**：C3 仍打标，但 **Big4 须 LONG 才开多**（`big4_not_long_blocks_C3`）。弱宏观 / 震荡 / 个币冲击不得穿透。8/26 近窗破位 C3 42 笔 42.9%/−1152U；8/23 多头窗曾 18 笔 94%/+726U。打标保留。  
...
> **冲高不过前高（v4.5.48 / v4.5.50）**：近 32–48 根 15m 已有高顶 → 回落 ≥0.40% → 第二次冲高低于前顶 ≥0.20% → `15m_failed_retest`，打 **B3**（Big4 仍 LONG 也允许）。禁止把第二次较低的顶当 A1 回踩。`entry_timing` 记 `failed_retest_short`；**须离高拒绝 K 才** `exhaustion_ready`。  
> **反弹不是回调（v4.5.47）**：近 16 根 15m 最后极值是**低点且已离开** → `15m_bounce_from_low`（反弹做空/观望）；最后极值是**高点且已离开** → `15m_pullback_from_high`（才允许 A1/B4）。禁止把死猫跳 / 缩量反弹识别成 A1 回调做多。`entry_timing` 记 `bounce_not_pullback`。  
> **单边不追高（v4.5.20 / v4.5.35 / v4.5.36）**：多单贴着近 8 根 15m 高点（距高 ≤**0.80%**）一律 `wait_pullback`；禁止仅凭 `15m_higher_low` / 缩量 / 靠近 EMA 在浅回调挂多。空头镜像：A2 贴低阴跌 / 仅缩量反抽不得挂空；B2 不得把第一根破位阴线当失败反抽。  
> **顺势不做空（v4.5.19）**：退出收口仍有效（到期不因 D1 强平；顾问 suggest-only）。开空闸门见 **v4.5.22**。  
...
> **BRAIN / 中线卖点（v4.5.16 / v4.5.34）**：B3/C4 **冲高滞涨**须见离开高点的拒绝 K 才 `exhaustion_ready`（离高 ≥0.28%，收在下半且上影/阴线）；贴高横盘 / RSI 仍超买不算衰竭；仍加速则 `wait_stall`；已离高点则 `missed_high`。  
> **BRAIN / 中线买入点（v4.5.15）**：方向对仍须等 **15m 回调/回抽进区** 才挂限价（禁止破位当根追高）；中线纳入 DeepSeek 持仓顾问（v4.5.19 起 sell 不执行）+ 更早锁利（峰≥1.2%），避免盈利变亏损。
...
| **INV-12** | **REQ-BRAIN**：硬 SL 与美元熔断仍兜底；**平仓主路径为结构点**（多单合适高点卖、空单合适低点平）；错过结构点或浮盈回吐须锁利，**禁止**把盈利单扛到硬 SL；**禁止** 4–6h 对亏损单计划到期强平。BRAIN 持仓顾问只建议不执行；仍**不得进入 SmartExit** | 平仓权责混乱 / 盈利变亏 |
...
### 2.1 Telegram 交易通知（v4.5.59）

**实现**: `trade_notifier.py` · `futures_trading_engine.py`（`_notify_paper_open` / 平仓 `notify_close_position`）

| 事件 | 是否发 TG |
|------|-----------|
| 模拟盘合约**开仓成交**（市价开仓或限价成交） | **发** `【模拟盘开仓】` |
| 模拟盘合约**平仓** | **发** `【模拟盘平仓】` |
| 实盘开仓 / 平仓 / 挂撤单 / 交易所 SL·TP 设置 | **不发** |
| 熔断、IP 封禁、每日复盘、币安公告 | **仍发**（运维告警，不是交易单） |

限价挂单未成交不发。`config.yaml` `notify_events` 用 `paper_open` / `paper_close`（旧 `live_open` / `live_close` 仅作别名，仍只触发模拟盘）。
...
| `midline_short_enabled` | 0 | 破位做空 kill switch；系统配置 / 破位策略页可开关 |
...
BRAIN / 破位 / DeepSeek / **自选手动** 实盘同一套开关：`live_trading_enabled=1` 才在**成交瞬间**排队同步；L1 模拟仓当场 `SKIPPED`。打开开关**不得**回填历史模拟仓（INV-01/02）。
...
- **破位策略** 杠杆/SL/TP 与模拟一致（SL **2.0–3.0%** / 无硬 TP / 5x；已开仓另加 −120U 熔断）  
...
| 标的池 | `config.yaml` **市值前 300** 的 Binance U 本位永续；破位策略只用其中 **前 100**；证券过滤、L2+/锁定禁止等既有闸门仍生效 |
...
| 持仓顾问 | **启用**（DeepSeek **结构点 thesis 复核**；**sell 只落库不执行**；硬 SL + 结构点 + 错过/回吐锁利兜底） |
| SmartExit | **排除**；平仓由 `position_sl_tp_monitor` |
| 移动止盈 | **结构点优先**，错过高低点/浮盈回吐走 `check_structure_profit_lock`。`midline_hold_exit` **不再执行** |
...
| kill switch | `midline_long_enabled` / `midline_short_enabled`（默认 0）；**系统配置「策略与模式」+ 破位策略页**可开关 |
| 限价超时 | **4h**（与扫描周期对齐） |
...
| 参数 | 值 |
|------|-----|
| 限价偏移 | **A1/A2/B3/C4 限价**：做多回调区 / 做空反抽区或冲高区；无区时回退 ±1%。**C1/C3/B2 市价跟风**（不挂限价） |
| 限价超时 | 扫描间隔×2（现网约 **30min**）；未成交取消 |
| SL / TP | **止损按 Playbook**：C1/B2 **2.0%**、C3 **2.5%**、A1/A2/B3/C4 **3.0%**；**无硬止盈**。已开 6% 单先吃 **−120U** 熔断 |
...
- **做多 C3（中线破位跟风 · v4.5.56）**：**须 Big4 LONG**，否则只打标（`big4_not_long_blocks_C3`）。放量突破阻力/1h 突破当根 **市价跟进**（`breakout_ready`）。假突破收回 → `invalidated`；已延伸 ≥1.40%（相对 **冲高前高点**，近 8 根 15m 不计入，避免突破位跟着涨）→ `missed_break`。**`rsi_extreme_high` + `near_7d_high`**，或极端 RSI 叠加滞涨（`stall_at_high` / `top_callback` / `long_upper_wick`）→ `chase_blowoff`（禁止市价追）。BRAIN 的 C3 **仍等回踩**（INV-11 限价），且同样须 Big4 LONG。
- **做多 A1（v4.5.49 / v4.5.51）**：须同时满足：1h EMA 多头 + HH/HL；15m **对近 32 根趋势新高回踩**（`15m_trend_high_pullback`：回踩的高点 ≥ 前 16 根高点，离高 **0.80%–3.50%**，未破前段低点）；缩量或下影；回到 EMA20 / 38–50% 区才挂限价。未就绪记 `wait_pullback` / `not_trend_high_pullback`。贴高浅跌、破位阴跌、`15m_higher_low` 单独都不够。**无趋势新高的从低点抬起** → `bounce_not_pullback`。**已有趋势新高回踩后的抬起仍是 A1**。**第二次冲高不过前高** → `failed_retest_short`（走 B3）。滞涨结构 → `stall_high_not_pullback`。
- **同币止盈冷却（v4.5.25）**：同 symbol、同方向，近 **4h** 已平仓且 `realized_pnl>0` → 破位/BRAIN 均不得再开（`recent_tp_cooldown`）。去重仍拦当前持仓；止盈后再追由本条拦。
- **做空 B3/C4（顶部回调点）**：先贴着近 8 根高点，再等 **第一根离开高点的拒绝/回调**：离高 **≥0.28%** 且收在 K 线下半（上影或阴线）。`15m_failed_retest` **只打标**，第二次顶上不得直接空；须同样离高拒绝才 `exhaustion_ready`。仍放量/收在上沿**破前顶** → `wait_stall`；只在高点横盘尚未真正离开 → `at_high_wait_first_callback`；已离高 ≥1.35% → `missed_high`。中线确认后 **挂限价空**；BRAIN 仍限价挂在高点上方 0.20–0.55%。**已有真实拒绝时允许 4h 仍偏多**；仅 `stall_at_high` / 停新高 / 贴 7 日高 / 单独不过前高 **不得**当衰竭。
...
| 机制 | 行为 |
|------|------|
| 硬 SL/TP | SL **2.0–3.0%**（按 Playbook）；**无硬 TP**；**−120U** 熔断。`position_sl_tp_monitor` |
| 到期 | **取消对亏损单按时长强平**。已有仓的 `planned_close_time` 对 BRAIN/破位忽略 |
| 程序化卖点 | **结构点优先**；错过高低点仍止盈；峰值回吐 / 满 4h 仍绿则锁。`midline_hold_exit` **不再执行** |
| 持仓顾问 | **启用** DeepSeek（**结构点 thesis 复核**；**sell 只建议不执行**；硬 SL / 结构点 / 锁利兜底） |
...
| **INV-BRAIN-04** | **开仓**：自有 Playbook 主判通过后直接开仓（**跳过开仓顾问**）。**平仓**：DeepSeek 持仓顾问复核并落库（**sell 不执行**）；硬 SL + 美元熔断 + **结构点出场**（§7.3.16；v4.5.57 取消计划到期） |
| **INV-BRAIN-05** | **平仓**：不以大脑战略翻转为主路径；持仓顾问只复核不执行；安全网为硬 SL、美元熔断、**结构高/低点**（不因 D1/翻转/4–6h 到期强平） |
...
| 项 | 约定 |
|----|------|
| 标的池 | **市值前 300**（`config.yaml` 按 CoinGecko 市值映射的 Binance USDT 永续；证券 / L2+ / 锁定剔除）。实盘开仓仍 **仅 L0** |
| **1H** | **大方向**；回看 **近 1 周** |
...
| 计划到期 | 达本笔 `planned_close_time`（评估 hold，≤8h）：浮盈≥0.8% 且未从峰值过度回撤 → 延 **2h**；否则 `planned_close_time_expired`。**不因** D1 / 场景翻转 / 取数失败强平 |
| 战略平仓 | **关闭**（不再因 Playbook FLAT / Big4 / 翻转主动平） |
| 持仓顾问 | **启用**（进入 DeepSeek **结构点 rubric**；结构未到不得因时长/浮盈 sell；**sell 只落库为 `advisor_suggest_only`，不执行平仓**） |
| 旧 ai-trail / soft-sl / trend-sl | **不对 BRAIN 生效**（仅用上述新版规则） |

原则：BRAIN 持仓期认 **硬 SL + −80U + 结构高/低点**；顾问只复核、不因 4h/浮盈卖。开仓仍跳过开仓顾问（Playbook 主判）。详情 §7.3.16。
...
| DeepSeek | 开仓**不参与**；持仓顾问只复核落库，**不执行平仓** |
...
| 持仓退出 | `structure_swing_exit`（多单卖高、空单平低）；DeepSeek 持仓顾问 **结构点 rubric**（不执行、不因 4h/浮盈卖）；硬 SL + BRAIN −80U；战略平仓/SmartExit 仍排除 brain |
| 旧路径（对照） | DeepSeek 探索/预测开仓**暂保留** |
| Web / API | `/brain_strategy`；`/api/brain-swing` |
| 回归 | `scripts/validate_brain_req.py` |

**实盘**：`brain_swing` ∈ `LIVE_SYNC_SOURCES`；开仓须 `live_trading_enabled=1` 且 **L0 白名单**；平仓须 `live_close_enabled=1`。模拟扫 **市值前 300**；L1/未评级成交瞬间 SKIPPED（仅 L0 进实盘）。打开开关不回填历史仓。
...
| ID | 名称 | 方向 | 一句话描述 |
|----|------|------|------------|
| **A1** | 多头趋势回踩 | LONG | **v4.5.51**：1h EMA 多头 + HH/HL + 对趋势**新高**回踩（`15m_trend_high_pullback`，离高 0.80%–3.50%，不破前低，缩量/下影）。回踩后从回踩低点抬起仍开。**禁止**无新高的死猫跳、不过前高、破位阴跌 |
| **A2** | 空头趋势反抽 | SHORT | 下降结构中离开低点进入反抽区，且出现上影/EMA 拒绝后再挂空。仅缩量或 lower_high 不够。**v4.5.36 拒绝确认后开仓** |
...
| **C4** | 向上假突多头陷阱 | SHORT | 假突破收回且须上影/缩量/衰竭确认；仅高点横盘不算 C4。入场同 B3 |
...
**B2 弱反抽失败做空 — 必选清单（v4.5.36）**：

1. 先有下跌或处于下降结构（不是大涨中的第一次回调）
2. 出现反抽，但「无力」≥ 2 条：缩量、涨幅浅、到不了前高/EMA、RSI 反弹弱、上影增多
3. 失败确认：15M 跌破反抽起点或再次跌破 EMA20 后 **跟风做空**
4. 禁止 B2：反抽明显放量且结构抬高（那是 B1/B4）；纯追跌无反抽（那是 C1）；`15m_lower_high` 单独不得同时充当「已反抽」和「已拒绝」

**A2 空头趋势反抽做空 — 必选清单（v4.5.36）**：

1. 下降结构：EMA 空头排列，且 1H LH/LL 或 1H 偏空
2. 价格已离开近 8 根低点 ≥0.25%，进入 EMA / 38–50% 反抽区
3. 拒绝确认：长上影、EMA 拒绝、假突破或 RSI 拐头（缩量 / lower_high 单独不够）
...
4. 等待：最后一根仍收在上沿**创新高（破前顶）** → `wait_stall`；贴高横盘尚未离开 → `at_high_wait_first_callback`
5. 放弃：已离高点 ≥ **1.35%** → `missed_high`；放量再破前高 → `invalidated`
6. 禁止 B3：4h 仍明确偏多且**没有**真实拒绝（趋势未尽，不在高点摸顶）
7. 禁止：把第二次较低的顶当成 A1「从高点回落做多」
...
| **结构** | `hh_hl` | 1H 高点/低点抬高 |
| | `lh_ll` | 1H 高点/低点降低 |
| | `15m_higher_low` | 15M 近 8 根低点抬高（**单独不够**当回调；反弹也会抬低） |
| | `15m_lower_high` | 15M 反抽不过前高 |
| | `15m_pullback_from_high` | 近 16 根最后极值是高点且已离开 ≥0.28%（真回调） |
| | `15m_bounce_from_low` | 近 16 根最后极值是低点且已离开 ≥0.25%（真反弹；禁止当 A1） |
| | `15m_failed_retest` | 近 32–48 根已有高顶，回落后第二次冲高不过前高（做空，禁止当 A1） |
| | `15m_trend_high_pullback` | 对近 32 根趋势新高回踩 0.80%–3.50% 且不破前低（A1 必选） |
...
> **仍跳过**：DeepSeek 开仓顾问。DeepSeek 持仓顾问用于 BRAIN 持仓 thesis 复核（**不执行平仓**）。
...
##### 7.3.16.3 结构点出场（v4.5.57；v4.5.60 补锁利）

实现：`entry_timing.compute_structure_exit` + `structure_swing_exit.py` + `position_sl_tp_monitor` BRAIN/破位分支。

| 阶段 | 条件 | 动作 |
|------|------|------|
| 未到点 | 持仓 &lt;20min | 只靠硬 SL / 美元熔断 |
| 多单卖点 | 15m `exhaustion_ready`（离高拒绝，同 B3/C4）且峰值 ≥**1.80%** | **市价平** `structure_sell_high` |
| 空单平点 | 15m `bounce_low_ready` 且峰值 ≥**1.80%** | **市价平** `structure_cover_low` |
| 错过高低 | A1 等：`missed_high` / `missed_low` 且仍浮盈 ≥**1.80%** | **市价平** `structure_missed_take`。**C1/C3/B2 不因 missed 平**（入场就在极值附近） |
| 浮盈回吐 | 峰值 ≥**2.20%** 现价 ≤+0.15% | **市价平** `structure_profit_to_loss` |
| 锁利 trail | 峰值 ≥1.80%（Big4 LONG 多单 2.50%）后回撤 ≥0.55%（多头 0.80%） | **市价平** `structure_trail_*` |
| 过期仍绿 | 持仓 ≥4h 且仍浮盈 ≥**1.50%** | **市价平** `structure_stale_take`（**不**对亏损单按时长强平） |
| 硬 TP / 5m 早撤 / 4–6h 亏单到期 | — | **关闭** |

**禁止**：BRAIN/破位走旧 `_check_ai_trail_tp` / `midline_hold_exit` / `planned_close_time_expired`。  
**运维**：`position_sl_tp_monitor` 仍回写 `max_profit_pct`。新开仓不写 TP、不写 `planned_close_time`。

##### 7.3.16.4 决策优先级（BRAIN / 破位持仓生命周期）

```text
1. brain_max_loss_usd（BRAIN：浮亏≤-60U）/ midline_max_loss_usd（破位：≤-120U）→ 平
2. 硬 SL → 平
3. structure_sell_high / structure_cover_low → 平
4. structure_missed_take / profit_to_loss / trail / stale_take → 平
5. 战略翻转 — 仍关；DeepSeek 持仓顾问 — 仅复核落库，sell 不执行
```
...
| 角色 | 路径 |
|------|------|
| 评估器 | `brain_risk_params.py`（SL 仍按币；hold/TP 不作为平仓时钟） |
| 开仓写入 | `brain_strategy_orchestrator._open_brain_entry` / `midline_explore_worker`（`planned_close_time=None`，`take_profit_pct=None`） |
| 结构出场 | `structure_swing_exit.py` + `entry_timing.compute_structure_exit` + `position_sl_tp_monitor` |
| 路由 | BRAIN/破位纳入 DeepSeek 持仓顾问（**结构点 rubric + suggest-only**）；仍排除 SmartExit |
| 回归 | `validate_brain_req.py` · `validate_midline_swing.py` |
...
| 不跟 | BRAIN 空单 / C3 突破跟多 / A2·B2·B3·C1·C4；破位 `midline_*`；自选 `manual_watchlist`；旧 DCA |
...
**回归**: `scripts/validate_watchlist.py`
...
**路由**: `open_advisor_routing.py` — 探索等统一 DeepSeek；BRAIN/中线 **跳过开仓顾问，纳入持仓顾问（仅建议、不执行平仓）**；BRAIN 退出见 §7.3.7 / §7.3.16
...
开仓顾问：中线 v2 / BRAIN **跳过**。持仓顾问：中线 v2 / BRAIN **纳入** DeepSeek（**结构点 rubric**；**sell 只建议不执行**；硬 SL/结构点兜底）。探索/预测持仓仍用 4h 窗口 + 浮盈保护。
...
| **REQ-BRAIN-HOLD / REQ-BRAIN-RISK** | **结构点出场** + **错过/回吐锁利**（须 ≥1.80%/2.20%，禁止碎盈）；取消 4–6h 亏单到期；破位 SL 2.0–3.0% + −120U；BRAIN −60U；顾问 suggest-only；§7.3（**v4.5.62**） |
...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-09-23 | **v4.5.62** | **修盈亏比**：锁利门槛抬到 1.80%/2.20%，不再收 0.5% 碎盈；破位 SL 改为 2.0–3.0%；已开 6% 单加 −120U 熔断；C1/C3/B2 不因 missed_low 平；每轮跟风最多 3 笔。需求文档: docs/REQUIREMENTS_LOGIC_ZH.md §7.2.3/§7.3.16.3 已同步 |
...
| 2026-09-14 | **v4.5.57** | **结构持仓**：BRAIN/破位取消 4–6h 到期与硬 TP/早锁利；多单低点买高点卖、空单相反（`structure_swing_exit`）。硬 SL 与 BRAIN −80U 仍兜底。需求文档: docs/REQUIREMENTS_LOGIC_ZH.md §7.2/§7.3.16 已同步 |
...
| 2026-08-23 | **v4.5.48** | **冲高不过前高做空**：已有高顶后再出现较低的第二次顶 → `15m_failed_retest` 打 B3 SHORT；禁止当 A1 回踩（`failed_retest_short`） |
| 2026-08-23 | **v4.5.47** | **反弹≠回调**：近 16 根 15m 区分 `15m_pullback_from_high` / `15m_bounce_from_low`；A1/B4 必须从高点回落；死猫跳+缩量不得打成回调做多（`bounce_not_pullback`） |
...
| 2026-08-21 | **v4.5.31** | **操作对照文档**：新增 `docs/BRAIN_AND_BREAKOUT_OPERATOR_ZH.md`（超级大脑 vs 破位）；逻辑条款未改 |
| 2026-08-21 | **v4.5.30** | **市值扫描池**：`config.yaml` 改为市值前 300 的 Binance U 本位永续；超级大脑扫 300；破位策略扫前 100；脚本 `sync_config_market_cap_symbols.py` |
| 2026-08-21 | **v4.5.29** | **破位策略开关上系统配置**：`midline_long/short_enabled` 出现在桌面/手机系统设置与破位策略页；关则停扫新开，已开仓仍监控 |
