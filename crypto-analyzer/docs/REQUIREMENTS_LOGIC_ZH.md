...
**手动锁定**（`trading_symbol_rating.rating_locked=1`）：黑名单管理页 `/symbol_blacklist` 手动添加/编辑时默认锁定；定时 `update_top_performers` **不覆盖**等级；`POST /api/rating/unlock` 解除后恢复自动规则。下架公告强制 L3 使用 `force=True` 可覆盖锁定。
