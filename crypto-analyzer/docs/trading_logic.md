...
5. 限价单待开仓自检（pendingValidation）⭐ 新增
...
- cancelOnTrendReversal: 检测是否发生了交叉信号（更严格）
...
当模拟盘限价单因自检/趋势转向/超时取消时，会同步取消对应的实盘限价单：

```python
if syncLive == True:
    # 查找实盘PENDING状态的订单
    # 调用 live_engine.cancel_order() 取消
    # 更新实盘持仓状态为 CANCELED
```
