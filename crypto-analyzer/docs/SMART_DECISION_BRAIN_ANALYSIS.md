...
### 优化内容:
1. ✅ **移除所有1D趋势信号** - 持仓时间4小时,不需要1D趋势判断
   - 移除: `trend_1d_bull`, `trend_1d_bear`
   - 移除对应的权重配置
   - 保留: 1D K线数据仅用于计算30日价格位置

2. ✅ **移除EMA评分维度** - Big4市场趋势判断已足够
   - 移除: `ema_bull`, `ema_bear` (已移除,功能重叠)
   - 原因: Big4 (BTC/ETH/BNB/SOL) 市场趋势检测已提供方向判断
...
4. ✅ **Big4趋势检测优化**
   - 力度公式: 价格变化% × 0.8 + 成交量归一化 × 0.2
   - 缓存机制: 15分钟检测, 1小时缓存
   - 评分调整: ±15/10/5分, 强烈冲突(≥60)直接跳过

### 信号组件更新:
- **总数**: 17个 → 14个 (移除2个1D, 移除2个EMA)
- **最高评分**: ~120分 (position_low + volume_power_bull + trend_1h_bull + consecutive_bull + breakout_long + momentum_up_3pct)
- **开仓阈值**: 35分
...
### 1.2 数据源
- **1日K线** (1d): 50根,仅用于计算30日价格位置 (不再用于趋势判断)
