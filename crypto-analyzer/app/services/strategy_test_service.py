                                    ...
                                    try:
                                        ...
                                        if volume_condition == '>1':
                                            ...
                                            if not (0.8 <= volume_ratio <= 1.0):
                                                volume_condition_met = False
                                                volume_reason = f"做空成交量不符合 (当前:{volume_ratio:.2f}x, 需要:0.8-1x)"
                                        elif volume_condition == '0.6-0.8':
                                            if not (0.6 <= volume_ratio < 0.8):
                                                volume_condition_met = False
                                                volume_reason = f"做空成交量不符合 (当前:{volume_ratio:.2f}x, 需要:0.6-0.8x)"
                                        elif volume_condition == '<0.6':
                                            if volume_ratio >= 0.6:
                                                volume_condition_met = False
                                                volume_reason = f"做空成交量不符合 (当前:{volume_ratio:.2f}x, 需要:<0.6x)"
                                        else:
                                            volume_condition_met = False
