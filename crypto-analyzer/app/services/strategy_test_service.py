...
class StrategyTestService:
    ...
    async def test_strategy(self, request: dict) -> Dict:
        ...
        try:
            ...
            try:
                ...
                # 预测入场配置
                predictive_entry = request.get('predictiveEntry', False)  # 启用EMA差值收窄预测入场
                predictive_entry_threshold = request.get('predictiveEntryThreshold', 0.15)  # 差值收窄到此阈值以内时入场
                predictive_entry_speed = request.get('predictiveEntrySpeed', 0.1)  # 差值收窄速度要求
                ...
                # 提前入场配置（预判金叉/死叉）
                early_entry = request.get('earlyEntry', {})
                ...
                # 持续趋势信号配置
                sustained_trend = request.get('sustainedTrend', {})
    ...
    async def _test_symbol_strategy(self, **kwargs) -> Dict:
        ...
        sustained_trend_min_strength = kwargs.get('sustained_trend_min_strength', 0.15)
        sustained_trend_max_strength = kwargs.get('sustained_trend_max_strength', 1.0)
        sustained_trend_require_ma10_confirm = kwargs.get('sustained_trend_require_ma10_confirm', True)
        sustained_trend_require_price_confirm = kwargs.get('sustained_trend_require_price_confirm', True)
        sustained_trend_min_bars = kwargs.get('sustained_trend_min_bars', 2)
        sustained_trend_cooldown_minutes = kwargs.get('sustained_trend_cooldown_minutes', 60)
            ...
            if time_point['type'] == 'buy':
                    ...
                    for lookback in range(1, lookback_count + 1):
                        ...
                        if prev_ema_short and prev_ema_long:
                            ...
                            if predictive_entry and buy_signal in ['ema_5m', 'ema_15m', 'ema_1h']:
                                ...
                                # 做多预测：EMA9 < EMA26（尚未金叉），但差值收窄到阈值以内
                                if 'long' in buy_directions and ema_short < ema_long:
                                    # 差值为负，检查是否收窄到阈值以内
                                    if abs(curr_diff_pct) <= predictive_entry_threshold:
                                        # 检查差值收窄速度：当前差值绝对值应该比前一个时间点小
                                        narrowing_speed = abs(prev_diff_pct) - abs(curr_diff_pct)
                                        if narrowing_speed >= predictive_entry_speed:
                                            signal_triggered = True
                                            buy_signal_triggered = True
                                            found_golden_cross = True  # 标记为金叉（预测）
                                            detected_cross_type = 'golden_predicted'
                                            debug_info.append(f"   🔮🔮🔮 EMA预测金叉入场！差值={curr_diff_pct:+.2f}%（阈值{predictive_entry_threshold}%以内），收窄速度={narrowing_speed:.2f}%（要求≥{predictive_entry_speed}%）")
                                            break

                                # 做空预测：EMA9 > EMA26（尚未死叉），但差值收窄到阈值以内
                                elif 'short' in buy_directions and ema_short > ema_long:
                                    # 差值为正，检查是否收窄到阈值以内
                                    if abs(curr_diff_pct) <= predictive_entry_threshold:
                                        # 检查差值收窄速度
                                        narrowing_speed = abs(prev_diff_pct) - abs(curr_diff_pct)
                                        if narrowing_speed >= predictive_entry_speed:
                                            signal_triggered = True
                                            buy_signal_triggered = True
                                            found_death_cross = True  # 标记为死叉（预测）
                                            detected_cross_type = 'death_predicted'
                                            debug_info.append(f"   🔮🔮🔮 EMA预测死叉入场！差值={curr_diff_pct:+.2f}%（阈值{predictive_entry_threshold}%以内），收窄速度={narrowing_speed:.2f}%（要求≥{predictive_entry_speed}%）")
                                            break

                            if buy_signal in ['ema_5m', 'ema_15m', 'ema_1h'] and not signal_triggered:
                                ...
                                # 如果启用了提前入场且当前没有检测到金叉/死叉，检查是否接近穿越点
                                    ...
                                    ema9_slope = 0
                                    ema9_slope_pct = 0
                                    if prev_ema_short and ema_short:
                                        ema9_slope = ema_short - prev_ema_short
                                    ...
                                    if 'long' in buy_directions and ema_short < ema_long:
                                        ...
                                        if early_entry_require_price_above_ema and curr_close:
                                            if curr_close <= ema_short:
                                                early_entry_conditions_met = False
                                                early_entry_reasons.append(f"价格未在EMA9上方(价格={curr_close:.4f}, EMA9={ema_short:.4f})")
                                        ...
                                        if early_entry_conditions_met:
                                            ...
                                            if curr_close:
                                                debug_info.append(f"   📊 当前价格={curr_close:.4f}, 价格/EMA9比={((curr_close/ema_short-1)*100):+.2f}%")
                                        else:
                                            # 预判条件不满足，记录原因（调试时可以看到）
                                            pass  # 避免输出太多日志
                                    ...
                                    # 4. 价格在EMA9下方（可选）
                                        ...
                                        if early_entry_require_price_above_ema and curr_close:
                                            if curr_close >= ema_short:
                                                early_entry_conditions_met = False
                                                early_entry_reasons.append(f"价格未在EMA9下方(价格={curr_close:.4f}, EMA9={ema_short:.4f})")
                                ...
                                if sustained_trend_enabled and not signal_triggered:
                                    ...
                                    if 'short' in buy_directions and ema_short < ema_long:
                                        ...
                                        # 条件2：MA10/EMA10趋势确认
                                        if sustained_trend_require_ma10_confirm and ma10 and ema10:
                                            if ema10 >= ma10:
                                                sustained_conditions_met = False
                                                sustained_reasons.append(f"MA10/EMA10未确认空头趋势")

                                        # 条件3：价格确认
                                        if sustained_trend_require_price_confirm and close_price:
                                            if close_price >= ema_short:
                                                sustained_conditions_met = False
                                        ...
                                        if sustained_trend_min_bars > 0 and current_buy_index >= sustained_trend_min_bars:
                                            ...
                                            for check_i in range(sustained_trend_min_bars):
                                                ...
                                                if check_idx >= 0 and check_idx < len(buy_indicator_pairs):
                                                    ...
                                                    if check_ema_short < check_ema_long:
                                                        bars_in_trend += 1
                                            if bars_in_trend < sustained_trend_min_bars:
                                                sustained_conditions_met = False
                                                sustained_reasons.append(f"趋势持续性不足({bars_in_trend}根 < {sustained_trend_min_bars}根)")
                                            ...
                                            for pos in positions:
                                                ...
                                                if pos_entry_time and pos.get('direction') == 'short':
                                                    if pos_entry_time >= cooldown_start:
                                                        sustained_conditions_met = False
                                                        sustained_reasons.append(f"冷却期内")
                                                        break
                                        ...
                                        if sustained_conditions_met:
                                            ...
                                            found_death_cross = True
                                            detected_cross_type = 'death'
                                            is_sustained_signal = True  # 标记为持续趋势信号
                                            debug_info.append(f"   📉📉📉 持续趋势做空信号触发！趋势强度={ema_strength_pct:.2f}%")

                                    # 检查是否处于持续多头趋势（做多机会）
                                    elif 'long' in buy_directions and ema_short > ema_long:
                                        sustained_conditions_met = True
                                        sustained_reasons = []
                                        ...
                                        if sustained_conditions_met:
                                            ...
                                            detected_cross_type = 'golden'
                                            is_sustained_signal = True  # 标记为持续趋势信号
                                            debug_info.append(f"   📈📈📈 持续趋势做多信号触发！趋势强度={ema_strength_pct:.2f}%")
                ...
                if buy_signal_triggered and can_open_position and not closed_at_current_time:
                    if len(buy_directions) > 0:
                            ...
                            if buy_volume_enabled and (buy_volume_short_enabled or buy_volume_short):
                                ...
                                if volume_condition:
                                    ...
                                    if volume_condition == '<1':
                                        ...
                                        if not (1.0 <= volume_ratio <= 2.0):
                                            volume_condition_met = False
                                            volume_reason = f"做空成交量不符合 (当前:{volume_ratio:.2f}x, 需要:1-2x)"
                                    elif volume_condition == '>2':
                                        if volume_ratio <= 2.0:
                                            volume_condition_met = False
                                            volume_reason = f"做空成交量不符合 (当前:{volume_ratio:.2f}x, 需要:>2x)"
                                    else:
                                        # 尝试解析为单一数值（兼容旧格式）
                                        try:
                                            required_ratio = float(volume_condition)
                            ...
                            if opposite_positions:
                                ...
                                for opp_position in opposite_positions[:]:
                                    ...
                                    trades.append({
                                        ...
                                        'time': self.utc_to_local(current_time),
                                        'balance': balance,
                                        'pnl': pnl,
                                        'pnl_pct': pnl_pct,
                                        'fee': close_fee,
                        ...
                        if macd_filter_enabled and not is_early_entry_signal and not is_sustained_signal:
                        ...
                        if kdj_filter_enabled and not is_early_entry_signal and not is_sustained_signal:
                        ...
                        # 检查 MA10/EMA10 信号强度（如果配置了）（预判信号和持续趋势信号跳过此过滤）
                        ma10_ema10_ok = True
                        if ma10 and ema10 and not is_early_entry_signal and not is_sustained_signal:
                            ...
                            if ma10_ema10_trend_filter:
                                ...
                                if not ma10_ema10_ok:
                                    ...
                                    continue
                        elif not is_early_entry_signal and not is_sustained_signal:
                            # 如果没有 MA10/EMA10 数据，记录警告（预判信号和持续趋势信号跳过此检查）
        ...
        if len(positions) > 0:
            ...
            for position in positions:
                ...
                trades.append({
                    ...
                    'price': exit_price,
                    'quantity': quantity,
                    'time': self.utc_to_local(last_time) if last_time else self.get_local_time(),
                    'pnl': pnl,
                    'gross_pnl': gross_pnl,
                    'pnl_pct': pnl_pct,
