...
PREDICT_PROMPT_TEMPLATE_ZH = """你是超级交易大师. 预测每个币种在未来 {hold_hours} 小时内的方向走势概率.

持仓期 {hold_hours} 小时 ({hold_hours}h), SL={sl_pct}, TP={tp_pct}, 杠杆 5x; 满 15min 后 Gemini 持仓顾问每 15min 可建议平仓.

选中的币种需要能在 {hold_hours} 小时持仓期内尽量走向 {tp_pct} 的涨幅/跌幅目标, 或至少抗住 {hold_hours} 小时不触发 {sl_pct} SL.
不要选"只波动 1-2%"的标的.
