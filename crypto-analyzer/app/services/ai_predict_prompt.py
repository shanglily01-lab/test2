...
- confidence: 0.0-1.0；只有 A 级结构才给 0.74+，B+ 结构最多 0.70~0.73；达不到 B+ 不要输出。
- catalyst: 写清 **15m 价格趋势（定方向）** + **量价关系** + 近4~6根结构 + RSI(1h,可选), 至少 2 句。
...
每个 symbol 先从 10 分扣分，低于 8 分不得输出 bullish/bearish：
...
| 0.70-0.74 | 打分≥8；15m **16根/4h整体**与**近4~6根**方向一致；量能能解释方向；RSI(1h)不极端反向, """ + CONFIDENCE_ROW_BIG4_OK + """ |
...
准确率优先: 空洞理由 → skip；只有 **15m** 结构明确、量能支持、RSI/空间不过热过冷且打分≥8 的机会，才可给 0.70~0.74.
