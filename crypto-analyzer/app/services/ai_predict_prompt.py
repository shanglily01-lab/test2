...
从列表中筛选未来 4 小时值得交易的机会；**不设数量上限**，所有满足方向清晰、量价支持、风险可控的机会都要输出 verdict。
...
从候选列表中筛选未来 4 小时值得交易的机会；**不设数量上限**，所有满足方向清晰、量价支持、风险可控的机会都要输出 verdict。
不要为普通 skip 标的输出 verdict；若没有任何可交易机会，输出空数组 `verdicts: []`。
- category: 'bullish' / 'bearish'；不要输出普通 skip 标的。
- confidence: 0.0-1.0；只有 A 级结构才给 0.74+，B+ 结构最多 0.70~0.73。
- catalyst: 写清 **15m 价格趋势（定方向）** + **量价关系** + 近4~6根结构 + RSI(1h,可选), 至少 2 句。
...
- 0.70~0.73 是边际可交易区，不是强信号；若存在 RSI 极端、贴近 7d 高/低点、量价背离、近端反向 K 线，必须剔除。
...
- 不设固定数量上限；凡是达到高质量开仓标准的机会都应输出，但不要为了凑数量输出边际机会。

# 开仓纪律（质量优先，但需要识别可交易机会）
- skip：15m 震荡且无突破/破位、量价明显矛盾、只有24h涨跌幅或资金费率
- conf=0.60~0.69 是观察区，不会开仓；若 **15m** 结构清楚且量能不反向，也必须达到更高质量才给 0.70+
- 禁止为「每个币都要有方向」而硬给 0.70；也禁止把明确 A- 结构全部压成 skip
...
| 0.70-0.74 | 15m **16根/4h整体**与**近4~6根**方向一致；量能能解释方向；RSI(1h)不极端反向, """ + CONFIDENCE_ROW_BIG4_OK + """ |
...
质量优先: 空洞理由 → skip；**15m** 结构明确、量能不反向且 RSI/空间不极端 → 可给 0.70~0.74.
