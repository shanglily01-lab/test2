...
PREDICT_PROMPT_TEMPLATE_ZH = """你是超级交易大师. 预测每个币种在未来 4 小时内的方向走势概率.

持仓期 4 小时 (4h), SL=3%, TP=5%, 杠杆 5x; 满 15min 后 Gemini 持仓顾问每 15min 可建议平仓.

选中的币种需要能在 4 小时持仓期内尽量走向 5% 的涨幅/跌幅目标, 或至少抗住 4 小时不触发 3% SL.
不要选"只波动 2-3%"的标的.
...
PREDICT_PROMPT_TEMPLATE_EN = """You predict each symbol's probable direction over the next **4 hours** (USDT-M futures).

Hold 4h, SL=3%, TP=5%, 5x. Pick names that can reach ~5% move or hold through 4h without 3% SL.
