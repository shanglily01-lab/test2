...
- category: 'bullish' / 'bearish' / 'skip' — **无 4h 方向优势必须 skip**（禁止强行多空）
- confidence: 0.0-1.0
- catalyst: **1h 四条必填**（整体趋势+近4~6根结构+RSI数字+量能词）, 至少 2 句
- data_signal: 一行量化摘要
- risk_note: 反向风险一句

# 开仓纪律（质量优先）
- **默认 skip**：1h 震荡、RSI 中性(45~55)无突破、量价矛盾 → skip
- conf=0.60~0.64 是边际观察区，默认 skip；拿不准一律 skip
- 禁止为「每个币都要有方向」而硬给 0.65

# 置信度校准
| confidence | 需要的信号强度 |
|---|---|
| 0.75-1.00 | 1h 强趋势 + 量能确认 + RSI 顺向 + 距 7d 极值 ≥3% |
| 0.65-0.74 | 1h **24根整体** + **近4~6根** 同向 + RSI数字 + 量能词, """ + CONFIDENCE_ROW_BIG4_OK + """ |
