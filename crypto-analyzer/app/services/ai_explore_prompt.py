...
# 主探索/预测开仓置信度 (质量优先；0.65+ 允许可交易 B+ 结构，不要求完美四维同振)
...
- **核心判据**：15m（16 根整体 + 近 4~6 根结构 + 量能）。
- **1h 保留**：仍提供 narrative 与 K 线表，用于**同窗口交叉验证**或更长趋势背景；**不得**用 1h 替代 15m 做核心 verdict。
"""

KLINE_15M_READING_BLOCK = """
## 15m K 线读法 (**核心判据**)
- **主证据**：`kline_narrative.15m` + 16 根 15m 表 — 对应未来 **4 小时**（本笔计划 {hold_hours}h）。
- **整体趋势**：近 **16 根 15m**（=4h 窗口）。
- **近期结构**：近 **4~6 根 15m**（回调/反弹/连阳连阴/突破回踩）。
- catalyst **必须**写清 15m 结构 + 成交量；RSI(1h)、7d 距离为**辅证**。
"""
...
## bullish/bearish 必须写清的核心证据（**15m 为主**）

1. **15m 整体趋势** — `kline_narrative.15m` 16根/4h 结论（偏多/偏空/震荡）
2. **15m 近期结构** — 近4~6根连阳/连阴/回踩/突破等（禁只看 1 根 15m）
3. **15m 成交量** — 放量/缩量/量平及与方向关系
4. **RSI(1h) / 7d 距离** — 辅证；有则写出，不得单独替代 15m

**可选（背景）**：一句 `kline_narrative.1h` 或近4根1h是否与15m同向 — **不能**代替 15m 四条。

## 可给 0.65+ 的情形

- 15m 整体+近结构同向，量能不明显背离；RSI(1h) 未极端反向。
- 近 4 根 1h 与 15m 同向 → 可加分；1h 更长背景逆势但 15m 结构清楚 → 仍可按 15m 给 0.65+（risk_note 写背景风险）。

## 禁止（→ skip）

- 主因仅 24h%/费率/Big4；无 **15m** 结构词
- **仅写 1h、无 15m**（后端拒）；或仅写 24 根 1h 整体而无 15m 明细

## data_signal 例

`15m:5连阳回踩缩量, RSI=58, 1h近4根同向`

## 自检

- catalyst 是否有 **15m结构+量能**？（必须）
- 1h 是否仅作一句交叉验证，而非唯一依据？
"""

EXPLORE_PROMPT_TEMPLATE = """你是超级交易大师. **未来 4 小时交易窗口**（本笔计划 {hold_hours}h）, SL={sl_pct}, TP={tp_pct}, 杠杆 5x; 满 15min 后持仓顾问每 15min 可建议平仓.

你的任务是: **15m 为核心判据**（16根=4h），结合量价、RSI(1h,辅证)、价格位置，判断未来 4 小时是否值得持有.
**1h 数据完整保留**（narrative 近24根 + 表），用于同窗口交叉验证（近4根1h≈16根15m）与更长趋势背景 — **不作核心 verdict**.
...
- confidence: 0.0-1.0
- catalyst: 写清 **15m 结构** + 量能词 + RSI(1h,可选), 至少 2 句具体技术依据
- data_signal: 一行量化摘要
...
- conf=0.60~0.64 属边际观察区，不会开仓；若结构已经清楚，不要卡在 0.64
- 系统开仓门槛：探索/预测 conf≥0.65；拿不准可以 skip，但不能把清楚的 15m 趋势延续全部压成 skip

# 置信度校准 (0.65+ 是可交易 B+，不是完美 A+)

| confidence | 条件 (全部满足才可给该档) |
|---|---|
| 0.75-1.00 | 15m 强趋势 + 量能确认 + RSI(1h)顺向 + 距 7d 极值 ≥3% 空间 |
| 0.65-0.74 | 15m **16根/4h整体**与**近4~6根**方向基本一致；量能持平/放大/缩量回踩能解释方向；RSI(1h)不极端反向 |
...
- 若 15m 16根与近4~6根同向、成交量支持，应给 bullish/bearish 0.65~0.72，而不是 skip 0.50~0.55。
...
**质量优先**: 空洞理由一律 skip；但对 **15m** 结构明确、量能不反向的机会，应果断给 0.65~0.72.
