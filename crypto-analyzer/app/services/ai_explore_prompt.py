...
- 允许全列表都是 skip；空机会比错误开仓更好。不要因为系统有开仓门槛就把边界单抬到 0.70。

# 先打分再给 confidence（必须内化执行，不需要额外字段）
每个 symbol 先从 10 分扣分，低于 8 分不得给 0.70+：
...
| 0.70-0.74 | 打分≥8；15m **16根/4h整体**与**近4~6根**方向一致；量能持平/放大/缩量回踩能解释方向；RSI(1h)不极端反向 |
...
- 若 15m 16根与近4~6根同向、成交量支持且 RSI/空间不过热过冷，才可给 bullish/bearish 0.70~0.74；边界单 skip。
...
**准确率优先**: 空洞理由一律 skip；只有 **15m** 结构明确、量能支持、RSI/空间不过热过冷且打分≥8 的机会，才给 0.70~0.74.
