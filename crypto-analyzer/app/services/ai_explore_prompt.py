...
BARS_1H_4H_WINDOW = 4
# catalyst OHLC 闸门：近 6 根局部 + 16 根/4h 整体
GATE_15M_RECENT_BARS = 6
...
def _normalize_ohlc_bars(raw: Optional[List[Any]]) -> List[dict]:
    """统一为 {o,c,v}；兼容 hub / kline_data / 顾问 ctx 字段名."""
    out: List[dict] = []
    for k in raw or []:
        if not isinstance(k, dict):
            continue
        try:
            if k.get("o") is not None and k.get("c") is not None:
                o = float(k["o"])
                c = float(k["c"])
                v = float(k.get("v") or 0)
            else:
                o = float(k.get("open_price") or k.get("open") or 0)
                c = float(k.get("close_price") or k.get("close") or 0)
                v = float(k.get("volume") or k.get("v") or 0)
        except (TypeError, ValueError):
            continue
        if o <= 0 or c <= 0:
            continue
        out.append({"o": o, "c": c, "v": v})
    return out


def _score_ohlc_for_side(bars: List[dict], side: str) -> dict:
    """客观统计阴阳根对方向的支持/反向（与开仓顾问同口径）."""
    empty = {
...
def _load_15m_ohlc_bars(symbol: str, limit: int = BARS_15M_4H_WINDOW) -> List[dict]:
    """优先 DataHub，失败再直查 kline_data（仅闸门用）."""
    sym = (symbol or "").strip()
    if not sym:
        return []
    try:
        from app.utils.futures_symbol import futures_symbol_rating_canonical
        sym = futures_symbol_rating_canonical(sym)
    except Exception:
        pass

    try:
        from app.services.binance_data_hub import get_global_data_hub, BinanceDataHub

        hub = get_global_data_hub()
        rows = []
        if hub is not None:
            rows = hub.get_klines_sync(
                sym, interval="15m", limit=limit, allow_rest_fallback=True,
            ) or []
        if not rows:
            rows = BinanceDataHub.rest_klines_emergency(sym, "15m", limit) or []
        bars = _normalize_ohlc_bars(rows)
        if bars:
            return bars
    except Exception as e:
        logger.debug(f"[catalyst-OHLC] hub 取 15m 失败 {sym}: {e}")
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT open_price, close_price, volume FROM kline_data "
                    "WHERE symbol=%s AND timeframe='15m' AND exchange='binance_futures' "
                    "ORDER BY open_time DESC LIMIT %s",
                    (sym, limit),
                )
                rows = list(reversed(cur.fetchall() or []))
            return _normalize_ohlc_bars(rows)
        finally:
            conn.close()
    except Exception as e:
        logger.warning(f"[catalyst-OHLC] DB 取 15m 失败 {sym}: {e}")
        return []


def _resolve_gate_side(*, category: str = "", side: str = "") -> str:
    s = (side or "").strip().upper()
    if s in ("LONG", "SHORT"):
        return s
    cat = (category or "").strip().lower()
...
def _15m_ohlc_direction_ok(
    sym_data: Optional[dict],
    *,
    category: str = "",
    side: str = "",
) -> Tuple[bool, str]:
    """真实 15m OHLC 复核方向；禁止仅靠 catalyst 文案过门."""
    gate_side = _resolve_gate_side(category=category, side=side)
    if gate_side not in ("LONG", "SHORT"):
        return True, ""
    ...
    if len(bars) < GATE_15M_RECENT_BARS:
        return False, "15m OHLC 不足，无法复核开仓方向"

    window = bars[-BARS_15M_4H_WINDOW:]
    recent = bars[-GATE_15M_RECENT_BARS:]
    score_all = _score_ohlc_for_side(window, gate_side)
    score_recent = _score_ohlc_for_side(recent, gate_side)

    if score_all["for"] <= score_all["against"]:
        return False, (
            f"15m OHLC {len(window)}根不支持{gate_side} "
            f"({score_all['summary']})"
        )
    if score_recent["for"] < score_recent["against"]:
        return False, (
            f"15m OHLC 近{GATE_15M_RECENT_BARS}根反向占优 "
            f"({score_recent['summary']})"
        )
    if score_recent["trail_against"] >= 2:
        return False, (
            f"15m OHLC 近端连续反向{score_recent['trail_against']}根 "
            f"({score_recent['summary']})"
        )
    if score_all["trail_against"] >= 3:
        return False, (
            f"15m OHLC 连续反向{score_all['trail_against']}根 "
            f"({score_all['summary']})"
        )
    return True, ""
...
def explore_catalyst_technical_ok(
    ...
    *,
    ...
    """开仓前校验: 文案门槛 + 真实 15m OHLC 方向; 拦截仅涨跌幅/费率."""
    ...
    ohlc_ok, ohlc_reason = _15m_ohlc_direction_ok(
        sym_data, category=category, side=side,
    )
    if not ohlc_ok:
...
# 仓位设置 (供你理解容错空间)
- 杠杆 5x, 名义本金 ~5000U, SL={sl_pct} 价格跌幅, TP={tp_pct} 涨幅
...
- category: 'bullish' / 'bearish' / 'skip' — 只有高质量 4h 方向优势才给 bullish/bearish；不确定、边界、证据不完整一律 skip
- confidence: 0.0-1.0
...
- conf=0.60~0.74 属观察/边际区，**不会开仓**；不得把边界单抬到 0.75
- 系统开仓门槛：探索/预测 conf≥0.75；拿不准一律 skip
- 允许全列表都是 skip；空机会比错误开仓更好。

# 先打分再给 confidence（必须内化执行，不需要额外字段）
每个 symbol 先从 10 分扣分，低于 9 分不得给 0.75+：
...
- catalyst 主要依据是 24h涨跌幅、资金费率、Big4 或“涨多/跌多”：直接 skip，confidence≤0.59

# 置信度校准 (0.75+ 才是可交易结构；后端另做真实 15m OHLC 复核)

| confidence | 条件 (全部满足才可给该档) |
|---|---|
| 0.75-1.00 | 打分≥9；15m 强趋势 + 量能确认 + RSI(1h)顺向 + 距 7d 极值 ≥3% 空间 |
| 0.70-0.74 | 打分≥8 但仍有瑕疵 — **观察，不开仓**（勿抬到 0.75） |
...
# DeepSeek 易错校准（必须遵守）
- 不要把“强势上涨/强势下跌”自动当成趋势延续。没有 15m 续量或回踩确认，必须降级到 skip 或 0.60~0.69。
- 追高风险、RSI偏高、贴近7d高点的 LONG，必须有“放量突破后缩量回踩不破”；否则 skip。
- 超卖风险、RSI偏低、贴近7d低点的 SHORT，必须有“缩量反弹失败或放量破位继续下杀”；否则 skip。
- 若 15m 16根与近4~6根同向、成交量支持且 RSI/空间不过热过冷，才可给 bullish/bearish 0.75+；边界单 skip。
- 全列表没有任何 bullish/bearish 是允许的；不要为了输出机会而放宽证据标准。
...
**准确率优先**: 空洞理由一律 skip；只有 **15m** 结构明确、量能支持、RSI/空间不过热过冷且打分≥9 的机会，才给 0.75+。
