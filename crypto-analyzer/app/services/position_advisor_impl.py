"""Position advisor shared implementation (neutral).

Open/hold prompt helpers, temper rules, market context, and close utilities.
DeepSeek/BRAIN reuse via advisor_core. Gemini open/hold LLM paths are offline stubs.
"""
...
_open_advisor_singleton: Optional["PositionAdvisorCore"] = None


def get_open_advisor() -> "PositionAdvisorCore":
    global _open_advisor_singleton
    if _open_advisor_singleton is None:
        from app.utils.config_loader import get_db_config
        _open_advisor_singleton = PositionAdvisorCore(get_db_config())
    return _open_advisor_singleton


class PositionAdvisorCore:
    """模拟仓顾问核心：prompt/复核/取价/平仓工具（Gemini LLM 路径已下线）"""
    ...
    def _get_current_price(self, symbol: str) -> Optional[float]:
        ...
        try:
            ...
            price = get_futures_trade_price(
                None, symbol, max_age_seconds=120, log_tag="顾问核心",
            )
            if price is not None and price > 0:
                return float(price)
        except Exception as e:
            logger.warning(f"[顾问核心] {symbol} 取价失败: {e}")

        logger.warning(f"[顾问核心] {symbol} 无有效市价，跳过本轮审核")
        return None
    ...
    def _fetch_klines_via_hub(self, symbol: str, interval: str, limit: int) -> list:
        ...
        try:
            ...
            logger.warning(f"[顾问核心] {symbol} Hub {interval} K 线失败: {e}")
            return []
    ...
    def _fetch_market_context(self, symbol: str) -> dict:
        ...
        if not ctx['klines_15m'] and not ctx['klines_5m'] and not ctx['klines_1h']:
            logger.warning(f"[顾问核心] {symbol} K 线全空（DB 过期且 REST 失败）")
        ...
        try:
            ...
            logger.warning(f"[顾问核心] 取市场上下文 {symbol} 失败: {e}")
    ...
    def _temper_losing_hold(
        ...
        if override:
            ...
            logger.info(f"[顾问核心] 亏损 hold 复核 → {action} ({override})")
        return action, reason
    ...
    def _build_prompt(position: dict, current_price: float, ctx: dict) -> str:
        ...
        k15 = PositionAdvisorCore._recent_klines(
            ctx.get('klines_15m', []), HOLD_15M_BARS,
        )
        k5 = PositionAdvisorCore._recent_klines(
            ctx.get('klines_5m', []), HOLD_5M_BARS,
        )
        k1h = PositionAdvisorCore._recent_klines(
            ctx.get('klines_1h', []), HOLD_1H_BARS,
        )
        klines_15m_str = PositionAdvisorCore._format_kline_table(k15)
        klines_5m_str = PositionAdvisorCore._format_kline_table(k5)
        klines_1h_str = PositionAdvisorCore._format_kline_table(k1h)
        s15 = PositionAdvisorCore._score_klines_for_side(k15, side)
        s5 = PositionAdvisorCore._score_klines_for_side(k5, side)
        s1h = PositionAdvisorCore._score_klines_for_side(k1h, side)
        loss_rules = PositionAdvisorCore._loss_tier_rules(roi_pct, side)
        loss_tier = PositionAdvisorCore._loss_tier_label(roi_pct)
...
"""
    ...
    def _build_open_prompt(
        ...
        return build_open_advisor_prompt(
            ...
            ctx=ctx,
            format_kline_table=PositionAdvisorCore._format_kline_table,
        )
    ...
    def review_open(
        ...
        """Gemini 开仓顾问已下线；恒跳过（由 DeepSeek 闸门审核）。"""
        return True, "gemini_open_advisor_offline"
        ...
        try:
            ...
            if result.get('success'):
                if not result.get('already_closed'):
                    logger.info(
                        f"[顾问核心] 模拟仓已关闭 id={position['id']} "
                        f"{position['symbol']} {position['position_side']} @ {close_price}"
                    )
                return True
            logger.warning(
                f"[顾问核心] 模拟仓平仓未成功 id={position['id']}: "
                f"{result.get('message', result)}"
            )
            return False
        except Exception as e:
            logger.error(f"[顾问核心] 关模拟仓异常 id={position['id']}: {e}")
            return False

    def _close_live_position(
        self, position: dict, reason: str, advisor_tag: str = "advisor",
    ) -> bool:
        """
        关闭模拟仓 + 主动平实盘。
        ...
        try:
            ...
            logger.warning(f"[顾问核心] 查实盘记录异常 id={position['id']}: {e}")
        ...
        if live_rows:
            try:
                ...
                for live_row in live_rows:
                    ...
                    if not target_key:
                        logger.warning(
                            f"[顾问核心] account_id={live_row['account_id']} 无活跃 API key, "
                    ...
                    try:
                        ...
                        if not result.get('success'):
                            logger.error(
                                f"[顾问核心] 平实盘失败 live_id={live_row['id']}: "
                                f"{result.get('error', '')}"
                            )
                            continue
                        ...
                        if cur2.rowcount > 0:
                            logger.info(
                                f"[顾问核心] live_futures_positions 已关闭 id={live_row['id']} "
                                f"{position['symbol']} pnl={live_pnl}"
                            )
                        cur2.close(); conn2.close()
                    except Exception as e:
                        logger.error(
                            f"[顾问核心] 平实盘异常 live_id={live_row.get('id')}: {e}"
                        )
                ...
                logger.error(f"[顾问核心] 批量平实盘异常 id={position['id']}: {e}")
    ...
    def tick(self) -> dict:
        ...
        return {
            ...
            'errors': 0,
            'closed': 0,
            'note': 'gemini_hold_advisor_offline',
        }


# Legacy name kept for scripts / old imports
GeminiPositionAdvisor = PositionAdvisorCore
