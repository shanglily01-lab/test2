...
每 4h 对候选池交易对调用 DeepSeek 预测未来 4h 方向,
根据预测结果直接开模拟单.
...
def _get_predict_symbols(conn) -> List[str]:
    ...
    if rows:
        ...
        for row in rows[:PREDICT_CANDIDATE_LIMIT]:
            ...
            symbols.append(sym)
        if symbols:
            logger.info(f"[DeepSeek预测] 从 candidate_pool_snapshot 获取 {len(symbols)} 个 symbol")
            return symbols
...
def _sync_to_live(
    ...
    if margin_ratio <= 0:
        logger.info(f"[DeepSeek预测] {symbol} 保证金比例={margin_ratio}, 跳过实盘同步")
        return
    ...
    for ak in active_keys:
        try:
            ...
            if act_margin < 5:
                logger.info(f"[DeepSeek预测] {symbol} 账号{ak.get('account_name','')} margin={act_margin:.1f}U < 5, 跳过")
                continue
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                (
                    ...
                    PREDICT_SOURCE, 'deepseek_predict', entry_reason,
                ),
            )
...
def _run_predict_round_body(triggered_by: str) -> Optional[int]:
    ...
    logger.info(
        f"[DeepSeek预测] === 一轮开始 (triggered_by={triggered_by}, "
