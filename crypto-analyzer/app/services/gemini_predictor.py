...
闸门:
  ...
  - 同 symbol+side 已有 OPEN gemini_predict 仓位 -> 跳过
"""
...
def _build_symbol_data(conn, symbol: str) -> Optional[Dict]:
    ...
    # 回退: 原逻辑 (15m 用 16 根/4h 叙事，与 candidate_pool 一致)
...
def _get_big4_signal(conn) -> str:
    try:
        ...
        logger.warning(f"[Gemini预测] 读 Big4 失败 (保守视为 NEUTRAL): {e}")
        return 'NEUTRAL'
...
def _sync_to_live(
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if not allowed:
                logger.info(f"[Gemini预测] {symbol} {reason}, 跳过实盘同步")
                conn.close()
                return
        conn.close()
    except Exception as e:
        logger.warning(f"[Gemini预测] 检查实盘闸门失败, 跳过实盘同步: {e}")
        return
    ...
    if margin_ratio <= 0:
        logger.info(f"[Gemini预测] {symbol} 保证金比例={margin_ratio}, 跳过实盘同步")
        return
    ...
    for ak in active_keys:
        try:
            ...
            if act_margin < 5:
                logger.info(f"[Gemini预测] {symbol} 账号{ak.get('account_name','')} margin={act_margin:.1f}U < 5, 跳过")
                continue
...
def _open_simulated_position(
    ...
    allowed, gate_reason = gate_simulated_open(
        ...
        hold_hours=get_ai_position_hold_hours(), account_id=PREDICT_ACCOUNT_ID, conn=conn,
    )
    if not allowed:
...
def _update_run_stats(conn, run_id: int, predictions_made: int, orders_opened: int) -> None:
    with conn.cursor() as cur:
        cur.execute(
            "UPDATE gemini_predict_runs SET predictions_made=%s, orders_opened=%s WHERE id=%s",
...
def _insert_verdicts(conn, run_id: int, verdict_rows: List[Tuple]) -> None:
    ...
    with conn.cursor() as cur:
        try:
            cur.executemany(
                """
                INSERT INTO gemini_predict_verdicts
            ...
            logger.error(f"[Gemini预测] 写入 verdicts 失败: {e}")
...
def run_predict_round(triggered_by: str = 'scheduler') -> Optional[int]:
    """跑一轮 Gemini 预测. 成功返回 run_id, 失败/跳过返回 None."""
...
def _run_predict_round_body(triggered_by: str) -> Optional[int]:
    ...
    try:
        ...
        logger.error(f"[Gemini预测] DB 连接失败: {e}")
        return None
    ...
    try:
        ...
        if failed_symbols:
            logger.warning(f"[Gemini预测] {len(failed_symbols)} 个 symbol 数据获取失败: {failed_symbols[:5]}...")
        ...
        logger.info(f"[Gemini预测] gemini 返回 verdicts={len(verdicts)}, summary={summary_zh[:80]}")
        ...
        logger.info(f"[Gemini预测] Big4 当前信号={big4}")
        ...
        for v in verdicts:
            ...
            if not tech_ok:
                ...
                predictions_made += 1
                logger.info(f"[Gemini预测] {symbol} 跳过弱 catalyst: {tech_reason}")
                continue
        ...
        try:
            ...
            logger.warning(f"[Gemini预测] Shadow 对比跳过: {_shadow_err}")

        logger.info(
            f"[Gemini预测] === 一轮结束 run_id={run_id} 开仓={orders_opened} "
        ...
        logger.error(f"[Gemini预测] 一轮异常: {e}", exc_info=True)
        try:
            elapsed = time.time() - t0
