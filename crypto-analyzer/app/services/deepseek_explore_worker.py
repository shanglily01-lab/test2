...
def _sync_to_live(
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if not enabled:
                logger.info(f"[DeepSeek探索] live_trading_enabled=0, 跳过实盘同步 {symbol}")
                conn.close()
                return
        ...
        logger.warning(f"[DeepSeek探索] 检查实盘开关/TOP50失败, 跳过实盘同步: {e}")
        return
                ...
                try:
                    ...
                    if notifier:
                        notifier.notify_open_position(
                            ...
                            margin=act_margin,
                            strategy_name=f'DeepSeek探索[{ak["account_name"]}]'
                        )
                except Exception:
                    pass
            else:
...
def _insert_verdicts(conn, run_id: int, verdict_rows: List[Tuple]) -> None:
    ...
    for row in verdict_rows:
        ...
        row_list[7] = (row_list[7] or '')[:30]
        if row_list[9] is not None:
            row_list[9] = str(row_list[9])[:255]
...
def run_explore_round(triggered_by: str = 'scheduler') -> Optional[int]:
    ...
    try:
        with _connect() as conn_chk:
            with conn_chk.cursor() as cur:
                cur.execute(
                    "SELECT MAX(asof_utc) AS last_run FROM deepseek_explore_runs WHERE status='ok'"
                )
                row = cur.fetchone()
                if row and row.get('last_run'):
                    elapsed_h = (asof_utc - row['last_run']).total_seconds() / 3600
                    if elapsed_h < 5:
                        logger.info(f"[DeepSeek探索] 上次成功运行距今 {elapsed_h:.1f}h < 5h, 跳过 (防重启重复)")
                        return None
    except Exception as e:
