...
# 中等波动币参数
NORMAL_MOVER = 8
NORMAL_MOVER_MIN_VOLUME = 5_000_000
NORMAL_CHG_MIN = 3.0
NORMAL_CHG_MAX = 15.0
EXCLUDED_EXTREME_SYMBOLS: set = set()
...
def _get_historical_stats(conn) -> dict:
    """查询 deepseek_explore 的历史表现用于 prompt 尾部反馈."""
    stats: dict = {
    ...
    try:
        ...
        logger.debug(f"[DeepSeek探索] 查历史统计失败: {e}")
    return stats
...
def _build_universe_fallback(conn, level3_set: set = set()) -> dict:
    ...
    if level3_set:
        ...
        if len(universe) < before:
            logger.info(f"[DeepSeek探索] 黑名单3级过滤(fallback): {before - len(universe)} 个交易对")
    ...
    with conn.cursor() as cur:
        for sym, sym_data in universe.items():
            try:
                ...
                logger.debug(f"[DeepSeek探索] enrich {sym} 失败: {e}")
                stale_syms.append((sym, f'enrich_exception:{e}'))

    if stale_syms:
        logger.warning(
            f"[DeepSeek探索] 剔除 {len(stale_syms)} 个 K 线断采的 symbol: "
...
def _get_big4_signal(conn) -> str:
    ...
    try:
        ...
        logger.warning(f"[DeepSeek探索] 读 Big4 失败 (保守视为 NEUTRAL): {e}")
        return 'NEUTRAL'
...
def _sync_to_live(
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            allowed, reason = check_live_open_allowed(symbol, DEEPSEEK_SOURCE, cursor=cur)
            if not allowed:
                logger.info(f"[DeepSeek探索] {symbol} {reason}, 跳过实盘同步")
                conn.close()
                return
        conn.close()
    except Exception as e:
        logger.warning(f"[DeepSeek探索] 检查实盘闸门失败, 跳过实盘同步: {e}")
        return
    ...
    try:
        ...
        logger.error(f"[DeepSeek探索] 获取实盘账号失败: {e}")
        return
    ...
    for ak in active_keys:
        try:
            ...
            if result.get('success'):
                logger.info(
                    f"[DeepSeek探索] 实盘下单成功 {ak['account_name']} {symbol} {side} "
                    f"margin={act_margin}U lev={act_lev}x "
                    f"paper_id={paper_position_id}"
                )
            else:
                logger.error(
                    f"[DeepSeek探索] 实盘下单失败 {ak['account_name']} {symbol}: "
                    f"{result.get('error', '')}"
                )
        except Exception as e:
            logger.error(
                f"[DeepSeek探索] 实盘下单异常 {ak.get('account_name','')} {symbol}: {e}"
            )
...
def _open_simulated_position(
    ...
    try:
        ...
        if qty <= 0:
            logger.error(f"[DeepSeek探索] {symbol} {side} 数量计算非正,跳过")
            return None
        ...
        logger.info(
            f"[DeepSeek探索] 开仓 {symbol} {side} @ {price:.6g} "
        ...
        logger.error(f"[DeepSeek探索] 开仓失败 {symbol} {side}: {e}")
        return None
...
def _insert_run(
    ...
    with conn.cursor() as cur:
        cur.execute(
            """
            INSERT INTO deepseek_explore_runs
            ...
            """,
            (asof_utc, DEEPSEEK_MODEL, universe_size, summary_zh,
...
def _update_run_trades_opened(conn, run_id: int, trades_opened: int) -> None:
    with conn.cursor() as cur:
        cur.execute(
            "UPDATE deepseek_explore_runs SET trades_opened=%s WHERE id=%s",
...
def _insert_verdicts(conn, run_id: int, verdict_rows: List[Tuple]) -> None:
    ...
    with conn.cursor() as cur:
        try:
            cur.executemany(
                """
                INSERT INTO deepseek_explore_verdicts
            ...
            logger.error(f"[DeepSeek探索] 写入 verdicts 失败: {e}")
            if safe_rows:
                sample = safe_rows[0]
                lengths = {
                ...
                logger.error(f"[DeepSeek探索] 首条 verdict 各字段长度: {lengths}")
            raise
...
def run_explore_round(triggered_by: str = 'scheduler') -> Optional[int]:
    """跑一轮 DeepSeek 探索. 成功返回 run_id, 失败/跳过返回 None."""
    if not _explore_running_lock.acquire(blocking=False):
        logger.warning(f"[DeepSeek探索] 上一轮还未结束, 跳过 (triggered_by={triggered_by})")
        return None
        ...
        try:
            with _connect() as conn_chk:
                due, due_reason = explore_round_is_due(
                    conn_chk,
                    strategy_key='deepseek_explore',
                    runs_table='deepseek_explore_runs',
                    next_due_key=DEEPSEEK_EXPLORE_NEXT_DUE_KEY,
                    now=asof_utc,
                    manual=False,
                    log_tag='DeepSeek探索',
                )
                if not due:
                    logger.info(
                        f"[DeepSeek探索] {due_reason}, 跳过 (triggered_by={triggered_by})"
                    )
                    _explore_running_lock.release()
                    return None
                explore_claim_next_slot(
            ...
            logger.warning(f"[DeepSeek探索] 调度检查失败, 保守跳过: {e}")
            _explore_running_lock.release()
            return None
    ...
    try:
        ...
        if historical_stats.get('total_trades', 0) >= 3:
            logger.info(
                f"[DeepSeek探索] 历史: {historical_stats['total_trades']}笔 "
        ...
        logger.info(f"[DeepSeek探索] Big4 当前信号={big4}")
        ...
        logger.info(f"[DeepSeek探索] 方向开关: allow_long={allow_long} allow_short={allow_short}")
        ...
        logger.error(f"[DeepSeek探索] 一轮异常: {e}", exc_info=True)
        try:
            elapsed = time.time() - t0
            _insert_run(
                conn, asof_utc, 0, '', elapsed,
