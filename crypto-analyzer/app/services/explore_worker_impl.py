"""Shared explore-worker implementation (neutral).

Universe build/enrich, global context, price helpers, and related utilities.
Historically lived in gemini_explore_worker.py; DeepSeek/prepared-bundle reuse
via explore_worker_common. Legacy Gemini run_explore_round remains for compat
(kill switch / routing offline).
"""
...
def _try_candidate_pool(min_volume: float = 1000000, limit: int = 100) -> Optional[List[Dict]]:
    ...
    try:
        ...
        logger.warning(f"[探索核心] 读取 candidate_pool 失败: {e}")
        return None
...
def _build_universe(conn) -> dict:
    ...
    if cached:
        ...
        if len(cached) < before:
            logger.info(f"[探索核心] 黑名单3级过滤: {before - len(cached)} 个交易对")

        # 缓存数据太少时回退 fallback（防止进程重启后缓存表刚清空或数据不全）
        if len(cached) < 20:
            logger.warning(f"[探索核心] 缓存候选池仅 {len(cached)} 个 (<20)，回退 kline_data JOIN")
        else:
            logger.info("[探索核心] 从 data_cache.candidate_pool_snapshot 读取候选池 "
                         f"({len(cached)} 个 symbol)")
            return _build_universe_from_cache(cached)

    # ── 回退: 原逻辑 (慢, 可能数分钟) ──
    logger.warning(
        "[探索核心] 无法从 candidate_pool_snapshot 读取候选池, "
...
def _build_universe_fallback(conn, level3_set: set = set()) -> dict:
    ...
    if level3_set:
        ...
        if len(universe) < before:
            logger.info(f"[探索核心] 黑名单3级过滤(fallback): {before - len(universe)} 个交易对")
    ...
    with conn.cursor() as cur:
        for sym, sym_data in universe.items():
            try:
                ...
                logger.debug(f"[探索核心] enrich {sym} 失败: {e}")
                stale_syms.append((sym, f'enrich_exception:{e}'))

    if pool_trusted and trust_pool_narratives:
        logger.info(
            f"[探索核心] 候选池 narrative 直用 {pool_trusted} 个 symbol (跳过 freshness 探针)"
        )
    if stale_syms:
        logger.warning(
            f"[探索核心] 剔除 {len(stale_syms)} 个 K 线断采的 symbol: "
...
def _call_gemini_explore(
    ...
    try:
        ...
        logger.error(f"[探索核心] Gemini 调用失败: {e}")
        return None, f"API: {e}"

    text = (resp.text or "").strip()
    logger.info(f"[探索核心] gemini 用时 {time.time()-t0:.1f}s, output_len={len(text)}")

    parsed, parse_err = parse_explore_llm_json(text, "探索核心")
    if parsed is None:
...
def _get_big4_signal(conn) -> str:
    ...
    try:
        ...
        logger.warning(f"[探索核心] 读 Big4 失败 (保守视为 NEUTRAL): {e}")
        return 'NEUTRAL'
...
def _sync_to_live(
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if not allowed:
                logger.info(f"[探索核心] {symbol} {reason}, 跳过实盘同步")
                conn.close()
                return
        conn.close()
    except Exception as e:
        logger.warning(f"[探索核心] 检查实盘闸门失败, 跳过实盘同步: {e}")
        return
    ...
    try:
        ...
        logger.error(f"[探索核心] 获取实盘账号失败: {e}")
        return

    if not active_keys:
        logger.info("[探索核心] 无活跃 API Key, 跳过实盘同步")
        return
    ...
    if margin_ratio <= 0:
        logger.info(f"[探索核心] {symbol} 保证金比例={margin_ratio}, 跳过实盘同步")
        return
    ...
    for ak in active_keys:
        try:
            ...
            # 按账号实际保证金计算数量
            if act_margin < 5:
                logger.info(f"[探索核心] {symbol} 账号{ak.get('account_name','')} margin={act_margin:.1f}U < 5, 跳过")
                continue
            ...
            if result.get('success'):
                logger.info(
                    f"[探索核心] 实盘下单成功 {ak['account_name']} {symbol} {side} "
                    f"margin={act_margin}U lev={act_lev}x "
                    f"paper_id={paper_position_id}"
                )
            else:
                logger.error(
                    f"[探索核心] 实盘下单失败 {ak['account_name']} {symbol}: "
                    f"{result.get('error', '')}"
                )
        except Exception as e:
            logger.error(
                f"[探索核心] 实盘下单异常 {ak.get('account_name','')} {symbol}: {e}"
            )
...
def _open_simulated_position(
    ...
    if is_symbol_blocked_level3(symbol):
        logger.warning(f"[探索核心] {symbol} 评级禁止开仓(L2+/锁定)")
        return None
...
def _insert_verdicts(conn, run_id: int, verdict_rows: List[Tuple]) -> None:
    ...
    with conn.cursor() as cur:
        try:
            ...
            logger.error(f"[探索核心] 写入 verdicts 失败: {e}")
            # 打印第一条记录的字段长度帮助排查
            if safe_rows:
                sample = safe_rows[0]
                lengths = {
                ...
                logger.error(f"[探索核心] 首条 verdict 各字段长度: {lengths}")
            raise
...
def run_explore_round(triggered_by: str = 'scheduler') -> Optional[int]:
    ...
    if not _explore_running_lock.acquire(blocking=False):
        logger.warning(f"[探索核心] 上一轮还未结束, 跳过 (triggered_by={triggered_by})")
        return None
    ...
    try:
        ...
        logger.error(f"[探索核心] 读 kill switch 失败,保守跳过: {e}")
        _explore_running_lock.release()
        return None

    if enabled_raw not in ('1', 'true', 'yes', 'on'):
        logger.info(f"[探索核心] kill switch=0, 跳过 (triggered_by={triggered_by})")
        _explore_running_lock.release()
        return None
    ...
    if not manual:
        try:
            with _connect() as conn_chk:
                ...
                if not due:
                    logger.info(
                        f"[探索核心] {due_reason}, 跳过 (triggered_by={triggered_by})"
                    )
                    _explore_running_lock.release()
                    return None
                explore_claim_next_slot(
            ...
            logger.warning(f"[探索核心] 调度检查失败, 保守跳过: {e}")
            _explore_running_lock.release()
            return None

    logger.info(f"[探索核心] === 一轮开始 (triggered_by={triggered_by}) ===")

    # 建立 DB 连接
    try:
        conn = _connect()
    except Exception as e:
        logger.error(f"[探索核心] DB 连接失败: {e}")
        _explore_running_lock.release()
        return None
    ...
    try:
        ...
        logger.info(f"[探索核心] run_id={run_id} 已登记 (status=partial, 进行中)")
        ...
        universe, global_ctx, from_shared = get_explore_prepared_bundle(
            conn, "探索核心", allow_rebuild=allow_rebuild,
        )
        universe_size = len(universe)
        logger.info(
            f"[探索核心] universe_size={universe_size} "
        ...
        if universe_size == 0:
            ...
            logger.warning("[探索核心] 候选池为空, 本轮结束")
            return run_id

        logger.info(
            f"[探索核心] 全局: Big4={global_ctx.get('big4_signal')} "
        ...
        if historical_stats.get('total_trades', 0) >= 3:
            logger.info(
                f"[探索核心] 历史: {historical_stats['total_trades']}笔 "
                f"胜率={historical_stats['win_rate']}% "
                f"总盈亏={historical_stats['total_pnl']}U"
            )
        else:
            logger.info(f"[探索核心] 历史: 样本不足 ({historical_stats['total_trades']}笔)")
        ...
        logger.info(f"[探索核心] Big4 当前信号={big4}")
        ...
        logger.info(f"[探索核心] 方向开关: allow_long={allow_long} allow_short={allow_short}")
        ...
        for v in verdicts:
            ...
            if not tech_ok:
                ...
                logger.info(f"[探索核心] {symbol} 跳过主观 catalyst: {tech_reason}")
                continue
            ...
            trades_opened += 1
            logger.info(
                f"[探索核心] 限价挂单已创建 {symbol} {side} order_db_id={position_id} "
                f"(待成交，非即时持仓)"
            )
            verdict_rows.append((
                run_id, symbol, db_category, confidence,
        ...
        try:
            ...
            logger.warning(f"[探索核心] Shadow 对比跳过: {_shadow_err}")

        logger.info(
            f"[探索核心] === 一轮结束 run_id={run_id} 开仓={trades_opened} "
        ...
        logger.error(f"[探索核心] 一轮异常: {e}", exc_info=True)
        try:
            elapsed = time.time() - t0
            _finish_run(
                conn, run_id, asof_utc, 0, '', elapsed,
