...
def log_advisor_review(
    ...
    *,
    ...
    """写入 gemini_advisor_reviews; 表不存在时静默跳过."""
    own = conn is None
    if own:
        try:
            conn = _connect()
        except Exception as e:
            logger.warning(f"[Gemini顾问记录] DB 连接失败: {e}")
            return None
    try:
        with conn.cursor() as cur:
            if not _table_exists(cur, "gemini_advisor_reviews"):
                return None
            cols = table_columns(cur, "gemini_advisor_reviews")
            data = {
            ...
            cur.execute(
                f"INSERT INTO gemini_advisor_reviews ({','.join(keys)}) VALUES ({placeholders})",
                tuple(data[k] for k in keys),
            )
            return cur.lastrowid
    except Exception as e:
        logger.warning(f"[Gemini顾问记录] 写入失败 {review_type} {symbol}: {e}")
        return None
    finally:
        if own and conn:
            try:
