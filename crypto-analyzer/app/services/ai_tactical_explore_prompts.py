...
def _tactical_family_label(defn: TacticalStrategyDef) -> str:
    if defn.key in ("pullback", "rebound"):
        return "回多反空"
    return "追涨杀跌"


def _tactical_sibling_block(defn: TacticalStrategyDef) -> str:
    """同族另一策略的完整边界说明（本 prompt 不输出，避免只写当前策略名）."""
    siblings = {
        "pullback": (
            "### 同族另一策略：反弹做空（本 JSON **禁止**输出，仅供对照）\n"
...
def _tactical_family_block(defn: TacticalStrategyDef) -> str:
    ...
    if defn.key in ("pullback", "rebound"):
        return (
            "## 战术族：回多反空（顺大势、逆小势）\n"
            "本族含 **两个独立子策略**（回调做多 + 反弹做空），调度上各跑一轮 LLM；"
            "**本轮你只负责其中一个**（见上文角色）。\n"
    ...
    return (
        "## 战术族：追涨杀跌（顺大势、顺小势）\n"
        "本族含 **两个独立子策略**（追涨做多 + 杀跌做空），调度上各跑一轮 LLM；"
        "**本轮你只负责其中一个**（见上文角色）。\n"
...
def _build_prompt(
    ...
    *,
    ...
    prompt = (
        f"你是加密货币 U 本位合约 **{family_label}** 战术族中的 **{definition.title_zh}** 专属分析师（非泛化多空评论）。"
        f"本轮只输出**符合下述「{definition.title_zh}」定义**的 {side_note} entry；"
        f"同族另一子策略及其它战术族形态一律不要塞进本 JSON。\n\n"
        f"{family}\n"
        f"{sibling_section}"
