...
端点:
- GET  /api/gemini-explore/bootstrap              首屏单请求 (status+runs+open+stats)
...
def bootstrap():
    ...
    try:
        ...
        data = explore_bootstrap_payload(
            ...
            enabled_key="gemini_explore_enabled",
        ...
        logger.error(f"[Gemini探索 API] /bootstrap 失败: {e}", exc_info=True)
...
def list_runs(limit: int = Query(20, ge=1, le=200)):
    ...
    try:
        with explore_db_cursor() as cur:
            runs = fetch_runs_list(cur, "gemini_explore_runs", limit)
...
def get_run_detail(
    ...
    try:
        ...
        with explore_db_cursor() as cur:
                row = fetch_run_detail_row(cur, "gemini_explore_runs", run_id, field)
        if not row:
...
def list_positions(
    ...
    try:
        with explore_db_cursor() as cur:
            if status == 'open':
                rows = fetch_open_positions(cur, "gemini_explore", limit)
            else:
                rows = fetch_closed_positions(cur, "gemini_explore", limit)
...
def list_positions_live():
    ...
    try:
        def _fetch():
            with explore_db_cursor() as cur:
                return fetch_open_positions(cur, "gemini_explore", 200)

        positions = explore_positions_cache_get("gemini_explore:open:live", _fetch)
...
def stats(days: int = Query(30, ge=1, le=365)):
    ...
    try:
        ...
        data = explore_stats_payload("gemini_explore", days=days)
        return {"success": True, "data": data}
    except Exception as e:
        logger.error(f"[Gemini探索 API] /stats 失败: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
