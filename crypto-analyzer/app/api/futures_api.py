...
@router.patch('/strategies/{strategy_id}/trade-direction')
async def set_strategy_trade_direction(strategy_id: int, request: Request):
    """
    设置策略的交易方向（正向/反向）
    ...
    try:
        body = await request.json()
        direction = body.get('direction')  # 'forward' 或 'reverse'
        enabled = body.get('enabled', False)

        if direction not in ('forward', 'reverse'):
            raise HTTPException(status_code=400, detail="direction 必须是 'forward' 或 'reverse'")
        ...
        try:
            ...
            logger.info(f"策略交易方向已更新: ID={strategy_id}, {dir_text}={status_text}")

            return {
                'success': True,
                'message': f'已{status_text}{dir_text}交易',
                'id': strategy_id,
                'direction': direction,
                'enabled': enabled
            }
