    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                ...
                counts = status_counts("gemini_predict")
...
async def list_runs(limit: int = Query(20, ge=1, le=200)):
    try:
        with explore_db_cursor() as cur:
            runs = fetch_predict_runs_list(cur, "gemini_predict_runs", limit)
...
async def get_run_detail(
    ...
    try:
        ...
        logger.error(f"[Gemini预测 API] /runs/{run_id}/detail 失败: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_positions(
    ...
    try:
        with explore_db_cursor() as cur:
            if status == 'open':
                rows = fetch_open_positions(cur, "gemini_predict", limit)
            else:
                rows = fetch_closed_positions(cur, "gemini_predict", limit)
...
async def list_positions_live():
    try:
        with explore_db_cursor() as cur:
            positions = fetch_open_positions(cur, "gemini_predict", 200)
...
async def stats(days: int = Query(30, ge=1, le=365)):
    """返回 Gemini 预测的累计盈亏统计."""
    try:
        data = explore_stats_payload("gemini_predict", days=days)
