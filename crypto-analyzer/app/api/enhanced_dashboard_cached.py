...
class EnhancedDashboardCached:
        ...
        try:
            ...
            for row in results:
                prices.append({
                    ...
                    'volume_24h': float(row['quote_volume_24h']) if row['quote_volume_24h'] else 0,
                    'high_24h': float(row['high_24h']) if row['high_24h'] else 0,
                    'low_24h': float(row['low_24h']) if row['low_24h'] else 0,
                    'trend': row['trend'],
        ...
        try:
            ...
            for row in results:
                ...
                recommendations.append({
                    ...
                    'scores': {
                        'total': float(row['total_score']) if row['total_score'] else 50,
                        'technical': float(row['technical_score']) if row['technical_score'] else 50,
                        'news': float(row['news_score']) if row['news_score'] else 50,
                        'funding': float(row['funding_score']) if row['funding_score'] else 50,
                        'hyperliquid': float(row['hyperliquid_score']) if row['hyperliquid_score'] else 50,
                        'ethereum': float(row['ethereum_score']) if row['ethereum_score'] else 50,
                    },
                    'data_sources': {
                        'technical': bool(row['has_technical']),
                        'news': bool(row['has_news']),
                        'funding': bool(row['has_funding']),
                        'hyperliquid': bool(row['has_hyperliquid']),
                        'ethereum': bool(row['has_ethereum']),
                    },
                    'data_completeness': float(row['data_completeness']) if row['data_completeness'] else 0,
    ...
    async def _get_funding_rates_batch(self, symbols: List[str]) -> Dict:
        ...
        try:
            ...
            for row in results:
                funding_rates[row['symbol']] = {
                    'funding_rate': float(row['current_rate']),
                    'funding_rate_pct': float(row['current_rate_pct']),
                    'trend': row['trend'],
    ...
    async def _get_hyperliquid_from_cache(self) -> Dict:
        ...
        try:
            ...
            """)
            monitored_count = cursor.fetchone()['count']
            ...
            cursor.execute("""
                ...
                LIMIT 5
            """)
            top_coins_data = cursor.fetchall()
            ...
            with HyperliquidDB() as db:
                ...
                trades_data = db.cursor.fetchall()
            ...
            for trade in trades_data:
                coin_display = token_mapper.format_symbol(trade['coin'])
            ...
            # 计算总交易量
            total_volume = sum(float(row['total_volume']) for row in top_coins_data)
