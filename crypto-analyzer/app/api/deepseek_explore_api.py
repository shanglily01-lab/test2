        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    "FROM deepseek_explore_runs WHERE id=%s",
                    (run_id,),
                )
                row = cur.fetchone()
        finally:
...
async def list_positions_live():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"[DeepSeek探索 API] /positions/live Hub 获取失败: {e}")
            hub = None
        ...
        for p in positions:
            ...
            if hub is not None:
                try:
                    ...
                    logger.debug(f"[DeepSeek探索 API] hub 取 {symbol} 实时价失败: {e}")
