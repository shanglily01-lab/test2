"""
DeepSeek 预测 HTTP API.

端点:
- GET  /api/deepseek-predict/status                  返回 kill switch + 最近一轮元数据
- POST /api/deepseek-predict/toggle                  切换 kill switch
- GET  /api/deepseek-predict/runs?limit=20           最近 N 轮运行记录
- GET  /api/deepseek-predict/verdicts?run_id=X       某轮的所有 verdicts
- POST /api/deepseek-predict/run-now                 手动触发一轮
- GET  /api/deepseek-predict/positions?status=open   当前 OPEN / 历史 CLOSED 仓位
- GET  /api/deepseek-predict/positions/live          实时盈亏
"""
...
async def status():
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    "WHERE setting_key='deepseek_predict_enabled' LIMIT 1"
                )
                row = cur.fetchone()
                ...
                cur.execute(
                    ...
                    "FROM deepseek_predict_runs ORDER BY id DESC LIMIT 1"
                )
        ...
        logger.error(f"[DeepSeek预测 API] /status 失败: {e}")
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    "VALUES ('deepseek_predict_enabled', %s) "
                    "ON DUPLICATE KEY UPDATE setting_value = %s",
                    (val, val),
                )
        finally:
            conn.close()
        logger.info(f"[DeepSeek预测 API] kill switch -> {val}")
        return {"success": True, "enabled": request.enabled}
    except Exception as e:
        logger.error(f"[DeepSeek预测 API] /toggle 失败: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_run_detail(
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                row = fetch_run_detail_row(cur, "deepseek_predict_runs", run_id, field)
        finally:
            conn.close()
        if not row:
        ...
        logger.error(f"[DeepSeek预测 API] /runs/{run_id}/detail 失败: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_verdicts(run_id: int = Query(..., ge=1)):
    try:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    "FROM deepseek_predict_verdicts v "
                    "WHERE v.run_id=%s "
                    "ORDER BY "
                    "  CASE v.action_taken "
                    "    WHEN 'opened' THEN 0 "
        ...
        logger.error(f"[DeepSeek预测 API] /verdicts 失败: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_positions_live():
    try:
        ...
        logger.error(f"[DeepSeek预测 API] /positions/live 失败: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
...
def _run_in_background(triggered_by: str):
    try:
        ...
        logger.error(f"[DeepSeek预测 API] 后台跑一轮异常: {e}", exc_info=True)
    finally:
        try:
            _run_lock.release()
        except Exception:
