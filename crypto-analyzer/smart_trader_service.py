...
class SmartTraderService:
    ...
    def open_position(self, opp: dict):
        """开仓 - 趋势策略已下线，保留接口供兼容调用。"""
        logger.warning(f"[DEPRECATED] open_position 已停用: {opp.get('symbol', '?')}")
        return False
            ...
            try:
                ...
                if (now - last_config_reload).total_seconds() >= 300:
                    try:
                        ...
                        if new_mp > 0 and new_mp != self.max_positions:
                ...
                # 4. 检查持仓（趋势/BTC动量开仓扫描已下线，仅监控）
                current_positions = self.get_open_positions_count()
                logger.info(
                    f"[STATUS] 持仓: {current_positions}/{self.max_positions} (monitor-only)"
                )
