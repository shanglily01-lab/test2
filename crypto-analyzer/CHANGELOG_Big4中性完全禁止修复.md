...
### Bug 1: 变量名错误 (commit a172a9a)
**问题**: 使用了未定义的变量 `score`，正确变量名是 `new_score`
**修复**: Line 3076, 3078, 3081 - `score` → `new_score`
**影响**: 修复后服务可以正常运行

### Bug 2: 逻辑冲突 (commit b0bc27e)
**问题**: Line 3115-3118旧逻辑期望所有NEUTRAL信号都被阻止，与新逻辑冲突
**表现**: `[LOGIC-ERROR] NEUTRAL信号不应到达此处,已在前面被阻止`
**修复**: 移除旧的error+continue逻辑，允许通过强信号过滤的NEUTRAL信号继续处理
**影响**: Big4中性时的强信号可以正常开仓
