...
try:
    ...
    if products_count > 0:
        cursor.execute("SELECT * FROM crypto_etf_products LIMIT 3")
        print("\n样本数据:")
    ...
    if flows_count > 0:
        cursor.execute("SELECT * FROM crypto_etf_flows ORDER BY trade_date DESC LIMIT 3")
        print("\n最近的数据:")
        for row in cursor.fetchall():
            print(f"  {row}")
    ...
    for trade_date in trade_dates:
        # 获取这一天的所有ETF数据，关联产品表获取asset_type
        cursor.execute("""
            SELECT
                p.asset_type,
                f.ticker,
        ...
        for flow in flows:
            ...
            holdings = Decimal(str(flow['holdings'])) if flow['holdings'] else Decimal(0)
