...
### 问题5: 切换按钮位置不当
**问题**: 切换按钮放在页面右上角不够直观
**优化**: 将按钮移至账户总览区域的card-header中
**原因**: 切换模式直接影响账户数据显示,放在账户总览处更符合逻辑
**Git Commit**: f412d64
...
1. **初始位置** (merge_futures_pages.py): 页面标题下方
2. **第一次调整** (4a49971): 移至页面右上角header-actions区域
3. **最终位置** (f412d64): 移至账户总览card-header中 ✅
...
┌─────────────────────────────────────────────────────┐
│  💼 账户总览                                          │
│           [模拟交易] [实盘交易]  最后更新: xx:xx:xx   │
│                                                       │
│  总权益: $xxx.xx    可用余额: $xxx.xx   ...          │
└─────────────────────────────────────────────────────┘
```

### 优势
✅ **逻辑清晰**: 切换模式直接影响下方显示的账户数据
✅ **操作便捷**: 在查看账户数据时可以随时切换模式对比
✅ **视觉关联**: 按钮与受影响的数据区域紧密相邻

---

**最后更新**: 2026-01-21
**Git Commit**: f412d64 (按钮位置优化)
