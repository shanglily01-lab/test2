...
if 存在Big4的活跃反弹窗口:
    logger.info(f"🚀 {Big4_symbol} 触发全市场反弹!")

    for symbol in 所有活跃交易对:
        if 无LONG持仓 and 无SHORT持仓 and 无冷却:
            开仓参数 = {
                'symbol': symbol,
                'side': 'LONG',
                'position_size_pct': 70,  # 🔥 激进仓位
                'take_profit_pct': 3.5,   # 🔥 激进止盈
                'stop_loss_pct': 2.5,     # 🔥 止损
                'strategy': 'emergency_bounce',
                'reason': f"市场反弹: {Big4_symbol}触底"
            }
            立即开仓(开仓参数)
```
...
### v2.1 (2026-02-09 逻辑修正)

**关键修正**:
1. ✅ **Big4作为信号源**: Big4触底 = 全市场信号
2. ✅ **全市场跟随**: 所有交易对都开多 (不限Big4)
3. ✅ **大周期过滤**: 加入72H/24H跌幅判断，过滤震荡假信号
4. ✅ **首次触底判断**: 24H内首次长下影线才触发
5. ✅ **参数优化**: 仓位70%, 止盈3.5%, 止损2.5% (基于实盘数据)

**用户反馈**:
- "你的止盈和止损设置的太过保守，抢反弹的时候，你看下2.6反弹的力度" → 止盈从2%提到3.5%
- "这种深V的持续下跌，不是说经常发生的，要结合大周期判断" → 加入72H趋势过滤
- "震荡周期经常出现也不算是大反转，反而让我们中招了" → 加入首次触底判断
- "Big4限制：只对BTC/ETH/BNB/SOL交易。你这个逻辑是错的，不是只交易4大天王，是只参考4大天王的信号，交易所有交易对，4大天王给出了趋势和方向了，当然是所有交易对都要冲" → **关键逻辑修正！**

### v2.0 (2026-02-09 优化版 - 已废弃)

**错误逻辑**: 只对Big4开反弹仓
**正确逻辑**: Big4作为信号源，全市场跟随
