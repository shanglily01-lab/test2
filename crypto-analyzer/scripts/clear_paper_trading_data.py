...
def clear_paper_trading_data():
    ...
    try:
        ...
        try:
            ...
            """)
            spot_accounts_reset = cursor.rowcount
            logger.info(f"✓ 现货账户: 已重置 {spot_accounts_reset} 个账户")
        ...
        logger.info("=" * 60)
        logger.info(f"✅ 数据清空完成！")
        logger.info(f"   共删除 {total_deleted} 条交易记录")
        logger.info(f"   重置了 {spot_accounts_reset + futures_accounts_reset} 个账户")
        logger.info("=" * 60)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='清空模拟现货和模拟合约交易数据')
    parser.add_argument('--yes', '-y', action='store_true', help='跳过确认，直接执行')
    args = parser.parse_args()
    
    if not args.yes:
        print("\n" + "=" * 60)
        print("⚠️  警告：此操作将清空所有模拟现货和模拟合约的交易数据！")
        print("=" * 60)
        print("将清空以下数据：")
        print("  - 所有订单记录")
        print("  - 所有持仓记录")
