"""
回填 ORDI/USDT 最近7天的历史数据
包括 1m, 5m, 15m, 1h, 1d 时间周期
"""
...
logger.remove()
logger.add(
    "logs/backfill_ordi_7days_{time:YYYY-MM-DD}.log",
    rotation="1 day",
    level="INFO",
...
async def backfill_ordi_7days():
    """回填 ORDI/USDT 最近7天的历史数据"""
    
    logger.info("=" * 80)
    logger.info("🔄 开始回填 ORDI/USDT 最近7天的历史数据")
    logger.info("=" * 80)
    ...
    start_time = seven_days_ago.replace(hour=0, minute=0, second=0, microsecond=0)
    end_time = now
    
    logger.info(f"⏰ 时间范围: {start_time.strftime('%Y-%m-%d %H:%M:%S')} ~ {end_time.strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info(f"📈 时间周期: 1m, 5m, 15m, 1h, 1d")
    ...
    backfiller.symbols = symbols  # 只采集 ORDI/USDT
    ...
    logger.info("\n🎉 ORDI/USDT 数据回补和缓存更新完成！")


if __name__ == '__main__':
    try:
        asyncio.run(backfill_ordi_7days())
