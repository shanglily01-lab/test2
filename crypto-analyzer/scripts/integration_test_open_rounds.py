#!/usr/bin/env python3
"""端到端开单集成测试：手动跑一轮 AI 策略，校验 DB 中 run 完成且 trades_opened/orders_opened >= 1。

非语法/ prompt 校验；需要 .env 中 DB + LLM API，会消耗 token 并在 account_id=2 写模拟仓。

用法:
  python scripts/integration_test_open_rounds.py --quick
  python scripts/integration_test_open_rounds.py --only gpt_explore,gemini_predict
  python scripts/integration_test_open_rounds.py --main --require-open
  python scripts/integration_test_open_rounds.py --full   # 全部策略，耗时长
...
def run_one(conn, spec: RoundSpec, require_open: bool) -> RoundResult:
    ...
    return RoundResult(
        spec.key, spec.label, "pass", run_id, opens, status, "", skip_summary,
        position_ids,
    )
...
def _print_report(results: List[RoundResult], require_open: bool) -> int:
    print("\n" + "=" * 72)
    print("开单集成测试报告", datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC"))
    print("=" * 72)
    fails = 0
    opens_total = 0
    for r in results:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="AI 策略开单端到端集成测试")
