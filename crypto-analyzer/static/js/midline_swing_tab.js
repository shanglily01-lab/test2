/**
 * 中线做多/做空 Tab — Gemini / DeepSeek 探索页共用
...
(function (global) {
  ...
  function panelHtml(prefix, title, source) {
    return (
      ...
      '<button type="button" id="' + prefix + '-run-now" data-source="' + esc(source) + '" class="px-3 py-1.5 rounded-lg bg-surface-container text-xs">立即运行</button>' +
      ...
      '</div></header>' +
      '<section class="px-8 py-6 grid grid-cols-4 gap-4">' +
      ...
      '<div class="bg-surface-container-low rounded-xl p-4"><p class="text-[10px] uppercase text-on-surface-variant mb-1">上轮池大小</p>' +
      ...
      '</section>' +
      '<section class="px-8 pb-6"><h2 class="text-sm font-bold uppercase tracking-widest text-on-surface-variant mb-3">运行记录</h2>' +
      ...
      '<th class="px-4 py-3 text-right">池</th><th class="px-4 py-3 text-right">信号</th>' +
    ...
    if (runBtn && !runBtn._midlineBound) {
      ...
      runBtn.addEventListener('click', function () {
        fetch(API + '/run-now?source=' + encodeURIComponent(source), { method: 'POST' })
          ...
          .then(function (d) {
            if (typeof global.showToast === 'function') global.showToast(d.message || '已触发', 'info');
          });
      });
    }
  ...
  function init(cfg) {
    ...
    var panels = [
      { tabId: 'midline-long', prefix: teacher === 'gemini' ? 'gml' : 'dml', title: (teacher === 'gemini' ? 'Gemini' : 'DeepSeek') + ' 中线做多', source: teacher + '_midline_long' },
      { tabId: 'midline-short', prefix: teacher === 'gemini' ? 'gms' : 'dms', title: (teacher === 'gemini' ? 'Gemini' : 'DeepSeek') + ' 中线做空', source: teacher + '_midline_short' },
    ];
    panels.forEach(function (p) {
      var el = document.getElementById('tab-' + p.tabId);
      if (!el) return;
      if (!el.dataset.midlineRendered) {
        el.innerHTML = panelHtml(p.prefix, p.title, p.source);
        el.dataset.midlineRendered = '1';
      }
  ...
  global.MidlineSwingTab = { init: init, onTabShow: onTabShow, loadAll: loadAll };
})(window);
